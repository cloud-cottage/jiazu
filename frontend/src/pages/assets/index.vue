 ...
 * 残页恒 `99 片兰帖残页`（负向同形，如 `-1 张兰帖`）—— 这三条文案与后台「资产变动日志」**逐字一致**
 * （两页共用同一函数，2026-09-26 实测）。
 * **另三类与后台不同形**（本页取量词：`+3 颗` / `+3 片` / `+3 枚`；后台取品类名：`+3 石榴籽` /
 * `+3 竹片` / `+3 石榴籽玉`）；碎片类两页同形（均为 `+3 碎片`）。
 * 该跨页差异在 `e84c3b1` 之前**即已存在**，非本次补单引入，本单**未改动**任一页文案。
 *
