...
### 13-14 反向指针：家谱 tab 组件化批次对 **§7-5 D3** 的取代（**追加 · 2026-09-24 · 只追加 · 上列各行原文一律保留**）

> **性质**：**追加式反向指针行** —— **未改代码、未写真源、未改 `AGENTS.md`**；本册 §1 – §13-13 的历史行**一字未动**，下列各项的语义**自本节起被覆盖**（口径 = `AGENTS.md` §0 第 4 条「历史版本行不机械改写」）。

1. **被取代项 = 本册 §7-5 的 `D3`**（「家族页（`family`）头图」，锚点 = `frontend/src/pages/family/index.vue:24` 的 `发源地：{{ hallInfo.origin }}`）：**2026-09-24 家族页（家谱 tab）单真源组件化批次**已把该页 **hero 卡整块移除** ⇒ **`D3` 这一展示落点在真机上不再存在**。
2. **不受影响 = 本册 §7-5 的 `D1`（首页卡片）/ `D2`（家族树页头）**：两者**继续读 `origin`**、**「无需改」的判定不变**；§13-13-6 第 7 条所列 `D1` / `D2` / `D3` 三档中**仅 `D3` 一档失效**，该条历史行**原文保留**。
3. **取代关系与逐条处置的唯一登记处** = `docs/home-sort-search.spec.md` **§11-4 第 1 行**（家谱 tab 口径真源 = 同册 **§11**）；人物地点册同项的处置见该册 **`D3`** 行（本册只作反向指针，**不复制其表**）。
4. **部署面**：家谱 tab 本批 **零后端改动** ⇒ **无云函数重打包项**；**前端 H5 + 小程序必须重打** ⇒ 登记 = `docs/PENDING_DEPLOY.md` **§32-7**（= 该册 §32-2 追加行）。

> **本节边界**：追加式登记 —— **未改代码、未写真源、未执行任何打包 / 部署 / 上传 / 重传、未 commit**；本册上列各行**一律原文保留**。

---

## 14. 行政区划热度排序（geo-hot）口径（**2026-09-26 · 只追加 · 不改 §1–§13 任何行**）

> **性质（只追加）**：本章为**纯追加** —— 本册 §1 – §13 既有行**原文一律保留、一字不改、一行不删**（口径 = `AGENTS.md` §0 第 4 条「历史版本行不机械改写」）。**本章只登记口径，不写实现现状读数**（文件行数 / 字节 / md5 / 测试条数 / 路由条数 /「已落盘」类状态**一律不写**，逐条执行时以对应命令的**实际输出**为准）。
>
> **权威口径文件** = 本批派单计划 `geo-hot/plan.md`（§1 裁定表 `R1`–`R8` / §2 冻结口径 `D1`–`D8`）；本章即其**逐条落盘**。§14-1 – §14-9 与 §1 / §2 的对应关系见各节标题后的括注。
>
> **终审落盘（Zang 终审 · 本批）**：本批交回的 **6 条待裁（`C1`–`C6`）** 与**实现方通报的 3 处交叉点**已由 Zang **终审裁定为 `R9`–`R15`**，本章**逐条逐字回填**（**不得自行调和、不得改口径**）。全文落点索引见 **§14-11**；正文受影响处**就地标注** `R9` – `R15`。
>
> **补充终审落盘（Zang 终审 · 本批第二轮）**：本批交回报告 §4 的 **5 条未调和项**已由 Zang **终审裁定为 `R16`–`R20`**（另有 **`R21` = 代码侧测试文件注释对齐**，**不属本册口径、本册不落**），本章**逐条逐字回填**（**不得自行调和、不得改口径**）。全文落点索引见 **§14-12**；正文受影响处**就地标注** `R16` – `R19`。
>
> **命名消歧（重要）**：本章引用的 `D1`–`D8` **一律指 plan.md 的冻结口径编号**，**与本册 §7-5 的展示落点 `D1` / `D2` / `D3`（首页卡片 / 家族树页头 / 家族页头图）毫无关系**；两者同名不同物，引用时须带册位（本册 §7-5 = 展示落点；本章 = 冻结口径）。

### 14-1 作用面与硬边界（承 plan.md `R4` / `R7` / `D7`）

- **唯一作用面 = `geo-cascader` 三级列表（省 / 市 / 县）的展示顺序**。除**列序**外**无任何行为变化**（选中值、回显、确认 / 清空逻辑、模板其余部分、样式**均不变**）。
- **一字不改**（真源与产物）：
  - 码表真源 `config/geo-divisions.json`；
  - 两份前端产物 `cloudfunctions/compat-api/lib/geo/divisions.json`、`frontend/src/business/geo/divisions.json`；
  - 生成脚本 `scripts/gen-geo-divisions.mjs`；
  - `frontend/src/pages.json`。
- **不改任何真源数据**（`migrate-output/**`、`config/**`）；**不加任何真源字段**（真源 schema 零变动）。
- **不新增集合**；**不新增任何写路由**；不动 `scripts/upload-migrated-to-cloudbase.mjs` 的 `COLLECTIONS`。
- **不新增页面 / 路由 / tab / npm 依赖**；不留调试开关 / 未使用 import / 死代码。
- **降级（硬）**：接口失败 / 无数据 / 未加载 ⇒ **一律原序**，**不报错、不阻塞、无任何 UI 提示** —— 无 loading、无「按热度排序」文案、无错误 toast。**选择器的可用性不依赖本接口**。

### 14-2 票源（三源 · 各计各的 · 只读派生）（承 plan.md `R2` / `D1`）

| # | 票源 | 取码路径 | 计票 |
|---|---|---|---|
| 1 | **出生地** | `people[handle].birth_place.origin_code` | 非空且通过 **14-3** 计入条件 ⇒ **1 票** |
| 2 | **居住地** | `people[handle].residence_places[]` 各项 `.origin_code` | 数组**逐项**：每项非空且通过 14-3 ⇒ **各 1 票** |
| 3 | **发源地** | 每棵树 **store 模块 `getMeta()` 返回值**（`meta.trees` 各项）的 `origin_code` | **每棵树 1 票** |

- **`R9` · tree-meta 的唯一读入口 = store 模块的 `getMeta()` 返回值**：**一律以 store 模块的 `getMeta()` 返回值为准** —— 它就是 tree-meta 的**唯一读入口**（**本地模式**读 `config/tree-meta.json` 真源、**云模式**读 `jiazu_tree_meta` 集合的 `global` 文档）；**表第 3 行原写的 `config/tree-meta.json` 是本地模式下的真源文件路径，不是第二个口径**。
- **`R9` · 计票字段（明示）**：= **`meta.trees` 下每项的 `origin_code`**（**不含 `storage_files`**）。
- **`R9` · 不在本批范围**：两处（`getMeta()` 视图与真源文件）**不同步**属**上云 / 同步批次**的问题，**不在本批范围**。
- **`R15②` · `getMeta()` 失败 ⇒ 只该源计 0 票**：**发源地那一源计 0 票**，**出生地 / 居住地两源照常计票**，**不整体失败**（**终审接受**）⇒ **不进** 14-6 的**整体失败降级**路径（单源读失败 ≠ 整体失败）。
- **遍历范围 = 全部树**：逐一取树；**取不到（不存在 / 已删）⇒ 跳过该树**；**任一树读失败 ⇒ 跳过该树继续**，仅记入失败树清单，**不得**因此整体失败（见 14-6 降级）。
- **`R17` · `failed` 的值形态 = 树 id 字符串数组**：`failed` **只收**「**树清单里有、但读取抛错**」的树（**树 id 字符串**）；「**不存在 / 已删**」（**单树返回 `null`**）**只跳过、不入 `failed`**。
- **`R16` · 树清单拉取失败 ≠ 成功但零票（硬）**：**算法扫真源时「树清单拉取失败」不得被当成「成功但零票」** ⇒ 它**必须与「整体计算失败」同路**（返回上一次成功值、无则空表降级、**不刷新成功时刻、不进缓存**、下次请求立即重试；见 14-6 / 14-7）。**单棵树读取抛错**仍**只跳过该树**（**进 `failed`**）、**单树返回 `null`（不存在 / 已删）**只**跳过**（**不进 `failed`**）—— **这两条不变**。
- **`R18` · `R15②` 与 `R9` 的张力（精确化 · 两层分开写）**：**①** `getMeta` **抛错** ⇒ **发源地那一源计 0 票**、**出生地 / 居住地两源照常**（**前提 = 树清单仍取得到** —— 本地模式下**树清单自带目录枚举回退**，故**通常成立**）；**②** 若**连树清单也取不到** ⇒ 按 **`R16`** 走**整体降级**。**两句话同时成立，不得只写一半。**
- **历史字符串形态兼容**：`birth_place` 为**历史字符串形态**时 —— **字符串不算票**（它没有码），**且不报错、不登记为异常**。
- **同源重复 = 已接受（本质批明示不计去重）**：同一码可因多源重复计票。**例**：某树树主出生地与 `tree-meta.origin_code` 同为 `371325` ⇒ 该码因**出生地 1 票 + 发源地 1 票**而**记 2 票**。**本批不做任何去重**。
- **零写入（硬）**：计票全程**不得调用任何写路径**（不得保存 meta / 保存树 / 集合写 / 树更新），**不得触发任何 `sweep` 类回写**；真源**零写入**。

### 14-3 计入条件（什么算票 / 什么不算票）（承 plan.md `D1` 末两条）

- **算票**：码为 **6 位数字**且**在码表内**（即 `lib/geo.js` 的 `isKnownOriginCode(code)` 为真）。
- **不算票且不报错、不登记为异常**：**空串** / **缺字段** / **非 6 位** / **未登记码**。这四种情况对 `counts` / `direct` 的数值与**最终列序不产生任何影响**。

### 14-4 计票口径 = 前缀（级联）（承 plan.md `R5` / `D2`）

- **定义**：一张票的码 `c` 对**其自身及全部祖先码各 +1**。候选祖先 = `[ c, c.slice(0,2) + '0000', c.slice(0,4) + '00' ]`，**去重**后**只保留在码表内者**（14-3 口径），逐个 +1。
- **算例（6 位县级码 ⇒ 展开成 3 个祖先码 · 逐字可判定）**：票 = **`371325`**（山东省 / 临沂市 / 兰陵县）⇒ **`370000` +1、`371300` +1、`371325` +1**（三级各 +1）。即该票在 `counts` 中贡献 **3 个单位**（每个码 1 票）。
- **边界算例**：
  - **`370000`（止于省）**：展开去重后仅 1 项 ⇒ **仅 `370000` +1**（市 / 县级祖先码不存在）。
  - **`110000`（直辖市）**：`slice` 结果与自身相同，**去重**后 1 项 ⇒ **仅 `110000` +1**。

### 14-5 备用读数 `direct`（只算恰在该级）（承 plan.md `R5`）

- **定义**：`direct` = **只算恰在该级的票**，**不做任何祖先展开** —— 码 `c` ⇒ **仅 `c` 自身 +1**；`c` 的祖先码**不因这张票增长**。
- **口径差异示例（与 14-4 同票对比）**：票 `371325` 在 `counts` 里给 `370000` / `371300` / `371325` **各 +1**；而在 `direct` 里**只给 `371325` +1**（`370000` / `371300` 的 `direct` 值不变）。
- **本批消费口径**：**前端只读 `counts`**；`direct` 为**备用读数**（同接口下发，留作后续校准 / 对账口径用）。
- **恒等关系（可判定 · `R13` 追认为正式口径）**：**6 位县级码无下级** ⇒ 其 `counts` 值 **恒等于** 其 `direct` 值。**`R13`**：本条与 **14-8 省级列序键**一并**追认为正式口径**（属 `D2` 前缀级联的**直接推论**，**不是新增条款**）。

### 14-6 接口契约（只读）（承 plan.md `D3`）

- **`GET /geo/hot` ⇒ 200**；**只读**（不得引入任何写副作用）；**无需鉴权**、**无需任何树标识头**（**无 `X-Tree-Id` 语义** —— 返回体为**全站汇总**，不区分树）。
- **顶层四键（成功路径恒在）+ 可选第五键 `failed`**（本批 = `v: 1`）：

```json
{
  "v": 1,
  "generated_at": "2026-09-26T11:50:00.000Z",
  "counts": { "370000": 7, "371300": 4, "371325": 2 },
  "direct": { "370000": 2, "371300": 1, "371325": 2 }
}
```

- **上例为成功路径**（四键齐备、无 `failed`）；`failed` 出现时**不改变其余四键的形态**。
- `v` = 协议版本（本批 `1`）；`generated_at` = **上次成功计算时刻**（ISO 8601 UTC）。
- **`R10` · `generated_at` 语义（终审）**：该字段语义 = **上次成功计算时间** —— **靠它才能区分「算了但没票」与「没算成」**。⇒ **成功但全站零票 ⇒ `generated_at` 给本次成功计算的时刻（不是 `null`）**；**`null` 专表「从未成功计算过」**。
- `counts` = **前缀（级联）票**；`direct` = **恰在该级的票**。
- **`R15①` · `failed` = 可选第五键**：顶层 = **四键（成功路径恒在）+ 可选第五键 `failed`**；**`failed` 仅当确有树读失败时出现**（**无树读失败 ⇒ 该键不出现**）。
- **`R15③` · 两映射的键序不作判据（硬）**：前端**按键取值、不依赖顺序** ⇒ **两映射（`counts` / `direct`）的键序不得写成判据**（否则下一轮质检会产生**无意义判负**）；实现里的「**票数降序 / 同票按码升序**」**只登记为内部表示**。
- **两表都只含票数 > 0 的码**（票 0 的码与未登记码**不出现在响应体中** —— **无键 ⇒ 前端按原序**，见 14-8）。
- **整体失败降级（硬）**：整体计算失败 ⇒ 返回**上一次的成功值**（若有）；**无上一次值** ⇒ `generated_at` 为**空值**（`null`）+ **两个空映射**（`counts` / `direct` 各为 `{}`），**仍 200**，**绝不 5xx、绝不把异常抛给前端**。**该降级值的缓存行为 = 不缓存**（见 14-7 `R11`）。
- **单棵树读失败 ≠ 整体失败**（14-2）：跳过该树继续，正常 200。
- **`R15②` · `getMeta()` 失败 ≠ 整体失败**：**发源地那一源计 0 票**，**出生地 / 居住地两源照常**，接口**照常 200**（**终审接受**）；该情形**不**走本节的整体失败降级路径。
- **`R17` · `failed` 的值形态（明示）**：`failed` = **树 id 字符串数组**，**只含**「**树清单里有、但读取抛错**」的树 id；「**不存在 / 已删**」**不入 `failed`**。⇒ `failed` 的**有无**不作成功 / 失败判据（`R15①`），其**内容**仅用于登记失败树。
- **`R16` · 树清单拉取失败 ⇒ 与整体计算失败同路（硬）**：**树清单拉取失败**属**整体计算失败**（**不得**当成「成功但零票」）⇒ 按本节的**整体失败降级**处理：返回**上一次成功值**（**无则空表降级**），**不刷新成功时刻、不进缓存**（见 14-7 `R11` / `R16`），**下一次请求立即重试**。**理由**：否则一次瞬时故障会把列序锁死一整天。
- **`R18` · `getMeta()` 抛错的边界（两层分开写）**：**①** `getMeta` **抛错** ⇒ **发源地那一源计 0 票**、**出生地 / 居住地两源照常**、**接口照常 200**（**前提 = 树清单仍取得到**）；**②** 若**连树清单也取不到** ⇒ 按 **`R16`** 走**整体失败降级**。⇒ 冒烟 / 质检**不得**把「单源读失败」（①）与「树清单取不到」（②）**混为一谈**。

### 14-7 缓存与新鲜度（承 plan.md `R3` / `D3` 末条）

- **24 小时 TTL 的进程内缓存**（`HOT_TTL_MS = 24 * 60 * 60 * 1000`）。
- **TTL 内不得重复扫描**：TTL 未过期的请求**直接复用缓存值**，**不得**重新遍历真源计票。
- **并发单飞**：并发到达的请求**共享同一份刷新 promise**（防缓存击穿），**不得**并发触发多次扫描。
- **`R11` · 降级值不进缓存、不设负缓存（硬）**：**整体失败**时返回**上一次成功值**（**无则空表**），**不刷新成功时刻** ⇒ **下一次请求立即重试**。**理由**：**否则一次瞬时故障会把列序锁死一整天**。
- **`R16` · 树清单拉取失败同样不进缓存（硬）**：**树清单拉取失败**与**整体计算失败**同路（见 14-6 `R16`；**不得**当成「成功但零票」）⇒ 返回**上一次成功值**（**无则空表降级**），**不刷新成功时刻、不进缓存**，**下一次请求立即重试**（**与 `R11` 同理由**：否则一次瞬时故障会把列序锁死一整天）。

### 14-8 排序规则（承 plan.md `R6` / `D5`）

- **主键：票数降序**（`count desc`）。
- **次级键：数据集原序** —— **显式以「数据集原序下标」做次级键**（比较 = `count desc` → `index asc`）；**不得依赖引擎 `sort` 的稳定性**。
- **必须返回新数组副本**：**不得**对 `provincesOf()` / `citiesOf()` / `countiesOf()` 的**返回数组原地排序**（模块内 `provinces` / `ownerIndex` 为共享索引，调用方不得修改）。
- **票 0 / 未登记码照原序**：无键（票 0）或计入条件不通过的码**保持数据集原序**（即「无票 ⇒ 原序」）。
- **映射的键序不作判据（`R15③`）**：排序**只按码在映射中取值**（`counts[code]`），**不依赖映射的键顺序**（键序 = 实现内部表示，见 14-6）。
- **三级各自独立**（各列用本列口径的票，不做跨级混合）：
  - **省级列**：排序键 = 该省码在 `counts` 中的值（前缀口径下该值 = **该省自身票 + 其下全部下级票之和** —— **`R13` 追认为正式口径**：属 `D2` 前缀级联的**直接推论**，**不是新增条款**）；
  - **市级列**：**在该省内**按该市码的 `counts` 值降序，同票回原序（**不跨省比较**）；**`R14`（终审）**：**市级列序键 = `counts`**（**前缀级联，含下属县票**），**与省级列同口径**；**`direct` 本批仅作备用读数、前端不消费** —— **理由**：真实数据里「**选临沂市最多**」**多半表现为填到某县**，**用 `direct` 会让市级列几乎不动、违背需求原意**；
  - **县级列**：**在该市内**按该县码的 `counts` 值降序，同票回原序（**不跨市比较**）。
- **无数据 / 接口失败 ⇒ 原序**（降级，见 14-1）。

### 14-9 未决（不阻塞 · 本批不做）（承 plan.md §5 待裁）

- **待裁 1：热度是否需时间衰减** —— **本批不做**。当前口径 = **全量条目等权计票、无时间窗**；历史票与新票**等权**。
- **待裁 2：是否需要「每用户去重」** —— **本批不做**。当前口径 = **按条目计票**（同一用户的出生地与其多条居住地**各计各的**）。
- 二者**均不阻塞本批**；若后续裁定，须**另起追加章节**并遵守「历史版本行不机械改写」（`AGENTS.md` §0 第 4 条）。

### 14-10 部署面（**只登记 · 本批不上云**）（承 plan.md `R8` / `D8`）

- 本批上云动作**只登记、不执行** ⇒ 唯一登记处 = `docs/PENDING_DEPLOY.md` **§39**。
- 登记项三类：**云函数 `compat-api` 重打包** + **H5 / 小程序两产物重打** + **冒烟三条**（细则与判据形态**只在该册 §39**，本章不复制其清单）。
- **不重打包的后果**：云端**无 `/geo/hot` 路由** ⇒ 前端按**降级原序**渲染（静默、无 UI 提示），**不报错**。

### 14-11 终审裁定回填（`R9` – `R15` · 逐条落点索引）（Zang 终审 · 本批）

> **性质**：本节 = 本批交回的 **6 条待裁（`C1`–`C6`）** 与**实现方通报的 3 处交叉点** 的**终审裁定逐条回填**。**逐条逐字回填，不得自行调和、不得改口径**；受影响处已在上文**就地标注** `R9` – `R15`（本节 = **索引 + 裁定原文**，**不新增、不改写任何数值口径**）。

- **`R9`（答 `C1` · tree-meta 以谁为准）** —— **落点：§14-2**（票源表第 3 行 + `R9` 三条明细）。
  - 一律以 store 模块的 `getMeta()` 返回值为准 —— 它就是 tree-meta 的唯一读入口（本地模式读 `config/tree-meta.json` 真源、云模式读 `jiazu_tree_meta` 集合的 `global` 文档）；`R2` 提到的 `config/tree-meta.json` 是本地模式下的真源文件路径，**不是第二个口径**。
  - 另需明确：**计票字段是 `meta.trees` 下每项的 `origin_code`（不含 `storage_files`）**。
  - 两处不同步属上云 / 同步批次的问题，**不在本批范围**。
- **`R10`（答 `C2` · 零票时的 `generated_at`）** —— **落点：§14-6**（`R10` 条）。
  - 成功但全站零票 ⇒ `generated_at` 给**本次成功计算的时刻**（**不是 `null`**）；`null` 专表「从未成功计算过」。理由：该字段语义 = 上次成功计算时间，靠它才能区分「算了但没票」与「没算成」。
- **`R11`（答 `C3` · 降级值是否进缓存）** —— **落点：§14-7**（`R11` 条）。
  - 不进缓存、不设负缓存：整体失败时返回上一次成功值（无则空表），**不刷新**成功时刻 ⇒ 下一次请求立即重试。理由：否则一次瞬时故障会把列序锁死一整天。
- **`R12`（答 `C4` · 两产物重打的判据形态）** —— **落点：本册 §14-10 所引的 `docs/PENDING_DEPLOY.md` §39 ②-1 / ②-2 / ②-3**（**判据清单只在该册 §39，本册不复制**）。
  - 接受你补的两条形态（判据落到冒烟第 2 条；冒烟不得以「改完立刻变」当判据），并补三条机械判据：**①** 云函数产物判据 = 在 `cloudfunctions/deploy/compat-api/index.js` 里 grep 命中 `geo/hot` 的计数 **≥ 1**；**②** H5 产物判据 = 在 `frontend/dist/build/h5` 下的 js 产物里同名 grep 计数 **≥ 1**；**③** 小程序产物判据 = 在 `frontend/dist/build/mp-weixin` 下的 js 产物里同名 grep 计数 **≥ 1**（三处具体文件名以实际产物为准）。
- **`R13`（答 `C5` · 你的两条推论）** —— **落点：§14-5**（恒等关系条）+ **§14-8**（省级列条）。
  - 追认为正式口径（属 `D2` 前缀级联的直接推论，**不是新增条款**）：**县级码的 `counts` 与 `direct` 恒等**；**省级列序键 = 该省自身票 + 其下全部下级票之和**。
- **`R14`（答 `C6` · 市级列序键）** —— **落点：§14-8**（市级列条）。
  - 市级列序键 = **`counts`**（前缀级联，含下属县票），与省级列同口径；`direct` 本批仅作备用读数、前端不消费。理由：真实数据里「选临沂市最多」多半表现为填到某县，用 `direct` 会让市级列几乎不动、违背需求原意。
- **`R15`（答实现方通报的 3 处交叉点）** —— **落点：① §14-6**（`failed` 条）、**② §14-2 + §14-6**（`getMeta()` 失败条）、**③ §14-6 + §14-8**（键序条）。
  - **①** `failed` 保留为**可选第五键**（仅当确有树读失败时出现）⇒ 口径表述改为「**顶层四键（成功路径恒在）+ 可选第五键 `failed`**」；**②** `getMeta` 失败 ⇒ **发源地那一源计 0 票、其余两源照常、不整体失败**（接受）；**③** 两映射的**键序不作判据**（前端按键取值、不依赖顺序），实现里的「票数降序 / 同票按码升序」**只登记为内部表示**，**不得写成判据**（否则下一轮质检会产生无意义判负）。

> **本节边界**：追加式登记 —— **未改代码、未写真源、未执行任何打包 / 部署 / 上传 / 重传、未 commit**；本册 §1 – §13 各行**一律原文保留**。

### 14-12 终审裁定回填（`R16` – `R21` · 逐条落点索引）（Zang 终审 · 本批第二轮）

> **性质**：本节 = Zang **本批第二轮终审**对 **`R16` – `R21`** 的**逐条逐字回填**（**逐条逐字回填、不得自行调和、不得改口径**）。受影响处已在上文**就地标注** `R16` – `R19`（§14 头注 + §14-2 / §14-6 / §14-7；`R19` 的本册登记位另见 §14-10 所引 `docs/PENDING_DEPLOY.md` §39 ③-1）。本节 = **索引 + 裁定原文**，**不新增、不改写任何数值口径**。**`R21`**（代码侧**测试文件注释对齐**）**不属本册口径**，本册**不落**，仅在此**登记其存在**。

- **`R16`（新裁定 · 真隐患的口径化）** —— **落点：§14-2**（`R16` 条）+ **§14-6**（`R16` 条）+ **§14-7**（`R16` 条）。
  - **算法扫真源时的「树清单拉取失败」不得被当成「成功但零票」**：它**必须与「整体计算失败」同路** —— 返回**上一次成功值**（**无则空表降级**），**不刷新成功时刻、不进缓存**，**下一次请求立即重试**（**与 `R11` 同理由**：**否则一次瞬时故障会把列序锁死一整天**）。
  - **单棵树读取抛错**仍**只跳过该树**（**进 `failed`**）；**单棵树返回 `null`（不存在 / 已删）**只**跳过**（**不进 `failed`**）；**这两条不变**。
- **`R17`（`failed` 的值形态）** —— **落点：§14-2**（`R17` 条）+ **§14-6**（`R17` 条）。
  - `failed` 的值形态 = **树 id 字符串数组**，**只收**「**树清单里有、但读取抛错**」的树；「**不存在 / 已删**」**不入 `failed`**。
- **`R18`（答本批交回报告 §4 第 1 条 · `R15②` 与 `R9` 的张力 · 精确化 · 两层分开写）** —— **落点：§14-2**（`R18` 条）+ **§14-6**（`R18` 条）。
  - **①** `getMeta` **抛错** ⇒ **发源地那一源计 0 票**，**其余两源照常**（**前提 = 树清单仍取得到** —— **本地模式下树清单自带目录枚举回退**，故**通常成立**）；
  - **②** 若**连树清单也取不到** ⇒ 按 **`R16`** 走**整体降级**。
  - **两句话必须同时写出，不得只写一半**（正文两处均已同时写出）。
- **`R19`（答本批交回报告 §4 第 4 条 · `§39` 冒烟判据与 `R10` 的张力）** —— **落点：`docs/PENDING_DEPLOY.md` §39 ③ 冒烟第 1 条**（**判据只在该册 §39，本册不复制**；本册登记位 = §14-10 所引 §39）。
  - **改掉** §39 冒烟第 1 条里「**`counts` 非空**」这个判据（**它与 `R10`「零票是合法成功态」冲突**）：改为 —— 接口返回 **`200`**，**顶层四键齐备**，**`generated_at` 非空（字符串）**，且 **`counts` 的键集 + 每个键的票数与「按真源独立现算」的结果逐键一致**（**可为空映射**）；**不以非空为判据**。
  - **为何改（已在 §39 ③-1 旁写明）**：为**区分「算了但无票」与「没算成」**。
- **`R20`（答本批交回报告 §4 第 5 条 · 登记位）** —— **落点：`docs/PENDING_DEPLOY.md` §39 ⑤-补**（**待执行项登记**）。
  - **线上真实 `generated_at` 的取证**属**上云冒烟执行单**，**不在本批本地阶段**；本册**只登记**该待执行项（**不预填读数**），登记位 = 该册 §39 的**未决 / 待执行**部分。
- **`R21`（不属本册口径 · 仅登记）** —— **代码侧测试文件注释对齐**，**不在本册落点**（本册口径不含测试文件注释）；**本册不落 R21**，仅在此登记其存在。

> **本节边界**：追加式登记 —— **未改代码、未写真源、未执行任何打包 / 部署 / 上传 / 重传、未 commit**；本册 §1 – §13 各行**一律原文保留**。

### 14-13 终审裁定回填（`R22` – `R23` · 逐条落点索引）（Zang 终审 · 本批第三轮 · 小补单）

> **性质**：本节 = Zang **本批小补单**对 **`R22` – `R23`** 的**逐条逐字回填**（**逐条逐字回填、不得自行调和、不得改口径**）。本节 = **索引 + 裁定原文**，**不新增、不改写任何数值口径**；本小补单为**纯追加** —— §14-1 – §14-12 既有行（含上文各条**就地标注**）**一律原文保留、未改一字**，本节即 `R22` – `R23` 的**登记位**。

- **`R22`（新裁定 · 整体降级路径的返回体形态）** —— **落点：§14-6**（`failed` 条的**降级形态**）+ **§14-7**（降级值的**返回体形态**）。**上述两处就地标注未动**（纯追加登记）。
  - **降级返回体不得新增、也不得修改 `failed` 键。** `failed` **只在成功路径出现**（语义 = **本次扫描**里读取抛错的树 id，见 `R17`）。
  - **整体失败时「本次扫描」不成立** ⇒ **直接原样复用上一次成功值**（它**自带当时的 `generated_at`**，以及**当时可能有、也可能没有的 `failed`**），**或**复用**空表降级常量**（该常量**不含 `failed`**）。
  - ⇒ **结论句（硬）**：**降级返回体不带「本次」语义的 `failed`**；其 **`generated_at` 随上次成功值，或为 `null`**（无上次成功值时的空表降级），**不得因降级而改写**。
- **`R23`（新裁定 · 「按真源独立现算」的固定步骤 + 环境 / 时点差口径 · 供 §39 冒烟用）** —— **落点：`docs/PENDING_DEPLOY.md` §39 ③-1 与 ③-3**（**冒烟判据只在该册 §39，本册不复制其清单**；本册登记位 = §14-10 所引 §39）。
  - **独立现算 = 固定步骤（逐条照做、不得自行增减）**：**①** 读真源的 **trees 全部树 JSON** 与 **tree-meta**；**②** 计票 = **出生地码 1 票** + **居住地每条 1 票** + **每棵树 tree-meta 的 `origin_code` 1 票**；**③** 每张票对其**自身**码与**祖先码**（**前 2 位 + `0000`**、**前 4 位 + `00`**）**各 +1**；**④** **只计在码表（`config/geo-divisions.json`）内**的码；**⑤** 产物 = **码到票数的映射**。
  - **时点差口径**：因 **24h 缓存** ⇒ **接口读数 = 上一次成功计算时刻的真源状态** ⇒ 两者**不一致**时**先查窗口内真源是否有写入**（**按仓内既有真源写入登记格式**），**不得直接判负**。

> **本节边界**：追加式登记 —— **未改代码、未写真源、未执行任何打包 / 部署 / 上传 / 重传、未 commit**；本册 §1 – §14-12 各行**一律原文保留**。
