...
- **（v2 · 2026-09-22 追加）家族树图不再绘制「外树子女镜像」** → 见 `docs/marriage.spec.md` **§9-7-11**（新口径 v2：判据 = `String(p.external_mirror) === 'true'` 且 `p.external_link_type === 'child'`，**该节点连同其子树整支不绘制**；**仅家族树** `kind === 'family'`（含 `kind` 缺省）适用，**世本 / 祖谱不适用**）。
  - **取代范围（旧行原文保留、不改写）**：本条**取代**本节「**二、拒绝语义**」中「**读路径 / 树图绘制**不因本守卫对**存量节点**二次拒绝（**存量照旧绘制**）」一句在「**家族树树图绘制**」上的语义 —— 即**外树子女镜像**在**家族树**树图内**不再绘制**；该句**其余部分**（守卫**只在写路径**生效）**不变**，本节「**一、写路径守卫口径**」「**三、边界与不变项**」其余全部内容**逐字不变**。
  - **状态**：前端实现 **由 Kong 实施中 / 由 Neng 质检**（**不表示已实现 / 已通过 / 已部署**）；上云登记 → `docs/PENDING_DEPLOY.md` **§30**（本批**无云函数重打包 / 无 `tcb fn deploy` 项**）。

---

## 11. 家族页（家谱 tab）单真源组件化口径（**追加 · 2026-09-24 · 只追加 · §1–§10 既有行原文一律保留**）

> **来源（用户需求原文 · 逐字）**：「@url:`http://localhost:5199/#/pages/family/index` 这个页面的现有内容可以移除掉，直接显示用户对应的家族树内容，即 @url:`http://localhost:5199/ji_23395_01` 中的内容即可。」（2026-09-24）
> **两条澄清（Kevin 2026-09-24 答）**：① 空态 —— **保留现有空态**（未登录 →「去登录」；已登录未加入 →「尚未加入」+ 去家族列表入口）；② 导航栏标题 —— **保持「我的家谱」**（**不动 `pages.json`**）。
> **状态（如实登记 · 三不）**：**Kong 已落盘 · 工作区在途未提交**；**前端 H5 / 小程序产物均早于本批**（H5 `2026-09-21 07:23:37` / 小程序 `2026-09-20 13:21:14` —— 均为本册时点实测）⇒ **未重打 / 未部署 / 未上线**；本节**只登记口径**，**不含任何验收结论**（验收归 Neng）。
> **性质**：追加章节 —— **§1–§10 与 §2-2 的既有行一律原文保留**；凡被取代处一律以「**已被取代 · 原文保留**」标注落地，**不机械改写历史行**（`AGENTS.md` §0-4 / §0-5）。

### 11-1 术语与落点（逐字 / 实测 · 2026-09-24）

| 术语 | 口径 | 实测落点 |
|---|---|---|
| **家谱 tab** | **tabBar 页** `pages/family/index`，页面标题 = **「我的家谱」** | `frontend/src/pages.json`：`:10` `"path": "pages/family/index"`、`:12` `"navigationBarTitleText": "我的家谱"`；`tabBar.pagePath` = `pages/family/index`（`:130`）；`tabBar` 段起于 `:119` |
| **数字馆（= 家族树首页）** | `pages/hall/index`；首页卡片入口 = `openTreeHome(tree_id)` | 入口见本册 **§2-1** |
| **共享组件（单真源）** | `frontend/src/components/tree-hall/tree-hall.vue` | **1055 行** · mtime **2026-09-24 18:28:55** |
| **家谱 tab 页面** | `frontend/src/pages/family/index.vue` | **89 行** · mtime **2026-09-24 18:29:14** |
| **数字馆页面（薄壳）** | `frontend/src/pages/hall/index.vue` | **23 行** · mtime **2026-09-24 18:29:07** |
| **地址栏别名链路** | 路径 / hash 别名（`/ji_23395_01`、`/z/`、`/z/<tree_id>`）由前端解析后 `uni.reLaunch({ url: '/pages/hall/index?tree_id=' + treeId })` | `frontend/src/App.vue:95`（`reLaunch` 唯一落点）；`MASTER_PATH = '/z/'` 见 `:18` |

> ⚠️ **易混澄清（逐字 · 不得混用）**：本册 **§10 的「列表 tab（家族 / 祖谱）」= 首页** `pages/index/index.vue` 的**档位切换**；**「家谱 tab」= tabBar 页** `pages/family/index`（标题「我的家谱」）。**两者不是同一个 tab、互不影响**（前者决定**首页**渲染哪一档列表；后者是底部 tabBar 的一个**独立页面**）。

### 11-2 单真源口径（硬约束 · 五条）

1. **家谱 tab 已登录且已绑定 ⇒ 整页只渲染同一份 `TreeHall`**（`pages/family/index.vue:28-33`：`:tree-id="boundTreeId"` + `:sync-nav-title="false"`）—— **禁止复制第二份模板**（两处注释逐字写明「**单真源，禁止复制模板**」：`hall/index.vue:1-9`、`family/index.vue:22-27`）。
2. **`pages/hall/index.vue` 变薄壳（23 行）**：只 `onLoad` 取 `tree_id` → `<TreeHall :tree-id="treeId" />`；**页面级生命周期只留在页面**（`tree-hall.vue:352-357` 注释逐字：「**组件内不得使用页面生命周期**（`onLoad` / `onShow` 仅页面可用）」）。
3. **prop 契约（逐字 · 默认值取自实现）**：`treeId?: string`（默认 `''`）、`syncNavTitle?: boolean`（默认 `true`）。
4. **导航栏标题**：家谱 tab 传 `syncNavTitle=false` ⇒ **不写标题**，保持 `pages.json` 的「我的家谱」（`tree-hall.vue:711-713`）；hall 页沿用默认 `true`（标题 = 家族名称）。**`pages.json` 一字未改**。
5. **换树 = 整棵重建**：`pages/family/index.vue` 对组件绑 `:key="boundTreeId"`（`boundTreeId = anchor.tree_id || ''`）；`onShow` 重取锚点后若换了树 ⇒ **重建组件**（组件内不碰页面生命周期）。

### 11-3 家谱 tab 页面组成（本批**移除项**逐条 · **空态文案逐字保留**）

- **新增取数方式（tab 页无 `onLoad` 参数）**：按登录态拉锚点 `fetchMyAnchor(token)`；`onMounted` + `onShow` 各拉一次；取不到（无 token / 未加入 / 请求失败）⇒ **保持 `null`** ⇒ 渲染「未加入」空态（**不打扰用户、不弹错**）。
- **移除（本批）**：① hero 卡（`display_title` / 谱名 / 堂号 / **发源地** / 简介 / 等级徽章）② 「我的身份节点」卡 ③ **导航格**（世系图谱 · 文献地址）④ 「**收录人物：N 人**」统计栏 ⑤ `loadError` 行。
- **保留（空态两组 · 逐字）**：① 未登录 = 🌳 + `登录后查看您加入的家族树` + `去登录`；② 已登录未绑定 = 🏡 + `您尚未加入任何家族树` + `加入后这里将显示您家族树的世系与档案` + `去家族列表加入`。

### 11-4 交叉引用（**受本批影响的既有口径** · 逐条给处置）

| # | 既有口径（**原文保留**） | 本批影响 | 处置 |
|---|---|---|---|
| 1 | `docs/geo-origin.spec.md` **§7-5 D3**：家族页（`family`）头图读 `hallInfo.origin`（登记时落点 = `pages/family/index.vue:24`）；该册尾部并重申「**D1 / D2 / D3 继续读 `origin`，「无需改」的判定不变**」 | **家谱 tab 的 hero 卡已随本批移除** ⇒ 家谱 tab 上**该展示点不再存在** | **旧行原文一律保留**；**取代关系自本行起**：D3 的「**家族页（`family`）**」一档**不再适用**；**首页卡片（D1）/ 家族树页头（D2）不受影响** |
| 2 | `docs/person-places.spec.md` **D3**（同上：`pages/family/index.vue:24` 的 `发源地：{{ hallInfo.origin }}`） | 同上 | 同上（该册**未改一字**，仅在此登记取代指向） |
| 3 | 本册 **§2-1**（卡片统一模板 / `openTreeHome(tree_id)` → `/pages/hall/index?tree_id=…`） | **不变** | — |
| 4 | 本册 **§10-4**（首页空态与建树入口） | **不变**（**不是**家谱 tab 的空态；家谱 tab 空态见 §11-3） | — |
| 5 | `docs/PENDING_DEPLOY.md` **§32-2**（前端重打面） | 本批前端**新增 3 个文件**（`components/tree-hall/tree-hall.vue` + `pages/family/index.vue` + `pages/hall/index.vue`） | 见同册 **§32-2 追加行**（本批家谱 tab **无后端改动项**） |
| 6 | `docs/economy.spec.md` **§14** / `docs/spirit-domain.spec.md` **§14** | **无交集**（行囊在「我的」页、玉归属在读侧） | — |

### 11-5 验收与部署口径

- **验收判据（交付时口径 · 本节不复述实现细节）**：家谱 tab 与直开 `/pages/hall/index?tree_id=…` 的家族树内容**逐字 / 逐项一致**；**两空态两组**；hall 原链路（`/ji_23395_01`、`/z/` 世本、`/z/<tree_id>` 祖谱）回归；`type-check` + `npm test` 不回归。
- ⚠️ **本册未实测（不预填）**：Jing 收口轮**未起栈、未做任何浏览器 / 几何读数** ⇒ 上述判据的**实测结论待 Neng 收口后回写**（本册**不写「已通过」**）。
- **上云**：本批家谱 tab **零后端改动** ⇒ **无云函数重打包项**；**前端 H5 + 小程序必须重打**（现产物早于本批）→ 登记 = `docs/PENDING_DEPLOY.md` **§32-2 追加行**。

### 11-6 D1 缺陷登记（**既有缺陷 · 已裁定修法 = 方案 A · 待修** · **追加 · 只追加 · §11-1 – §11-5 既有行原文一律保留**）

> **性质**：**缺陷登记行**（**只追加**）—— 归本册 §11；**未改代码、未写真源、未执行任何部署**；**修复单已派**（另见 `docs/PENDING_DEPLOY.md` **§32-7** 同批次）。

| 项 | 内容（**逐字几何 · 实测**） |
|---|---|
| 现象 | 树内搜索【搜索】按钮在 **1280×633** 视口、**未滚动**时被**固定图例栏完全遮挡**，**真实鼠标点不到**（点击中心命中「📐 横向」图例按钮），**键盘 Enter 也不触发** ⇒ **默认视口下搜索不可达**；**滚动 200px 后可点**、返回 **20 行**结果 |
| 逐字几何 | `{"viewport":[1280,633],"scrollY":0,"searchBtn":[1208,327,52,32],"rail":[1206,219,62,196],"topAtBtnCenter":"UNI-VIEW.layout-btn","railCoversBtn":true}`；`elementsFromPoint` = `UNI-VIEW.layout-btn` → `UNI-VIEW.layout-switcher(fixed,z:900)` → `UNI-BUTTON.t-button`；**hall 直开页几何与命中完全相同** |
| 根因 / 代码锚点 | `tree-pedigree.vue` **:39** + **:869**（`.layout-switcher{position:fixed;right:12px;top:50%}`）压住 `tree-hall.vue` **:57**（`.search-box`，CSS **:916**） |
| 严重度 | **medium** |
| 性质 | **既有缺陷**（**HEAD 版即如此，非本批引入**）但**已在本批新建的家谱 tab 上同样暴露** |
| 裁定 | **方案 A（只给搜索行加右侧避让，不动图例栏）** |
| 状态 | **待修** —— 修复单已派（另见 `docs/PENDING_DEPLOY.md` **§32-7** 同批次） |

### 11-7 追记 · 方案 A 已实施并实测通过（2026-09-25）（**追加 · 只追加 · §11-1 – §11-6 既有行原文一律保留**）

> **与 §11-6 的关系（先例：§0 第 4 条「追加追记行 + 旧行原文保留」）**：§11-6 表格各行 —— 含「**裁定 = 方案 A**」「**严重度 = medium**」「**状态 = 待修** —— 修复单已派」—— **原文一律保留，不删、不改、不重写措辞**；**其语义自本追记起被覆盖**：D1 现状 = 「**已实施并实测通过**」，实施路径即 §11-6 裁定的**方案 A**。§11-6「现象」行末尾「**真实鼠标点不到**（点击中心命中「📐 横向」图例按钮）」的**直接后果口径**，亦被下方「**现象/严重度更正**」条取代（实测直接后果 = **误触图例栏第三个按钮并跳页**，见下）。
> **溯源与纪律**：本块数值来自**独立实测**（独立副本栈 **5350 / 5360 / 9335**，**真源与用户栈未碰**），**逐字引用、未重算、未改写**；本块为**纯追加**（该文件**删除列 = 0**），**未改代码、未改其它文件、未 commit**。

**追记 · 方案 A 已实施并实测通过（2026-09-25）**：

- 实施点：`frontend/src/components/tree-hall/tree-hall.vue` **:920** `.search-box { … padding-right: 74px; }`（注释 :916–:919）；**未改** `tree-pedigree.vue` 的 `.layout-switcher`、未改模板、未改其它文件；
- 实测（1280×633 未滚动）：搜索按钮 `[1134,346,52,32]`（修前 `[1208,346,52,32]`）⇒ `elementFromPoint(1160,362)` = `UNI-BUTTON.t-button`（修前 = `UNI-VIEW.layout-btn`）；真实鼠标点击 ⇒ `.search-panel` 出现、`.search-result` **10 行**、hash 不变；家谱 tab（`#/pages/family/index`）同组件同样可点、**20 行**；
- 窄视口 **390×844**：搜索行 `[20,368,350,58]`、按钮 `[243,381,53,33]` 命中按钮本体、真实点击 10 行、`scrollWidth = innerWidth = 390` 无横向溢出；
- **像素级回归对照（A/B 同会话切 CSS）**：除 `padding-right` 与输入框矩形（`[20,334,1180,56]` → `[20,334,1106,56]`）外，`searchBox/hero/legend/viewBody/chart` 矩形、`docScrollHeight 961`、`canvasCount 1`、**canvas 墨迹像素数 11991**、墨迹 bbox `[733,36,794,429]`、**像素哈希 798785064** 逐位一致 ⇒ 无纵向位移、树图/图例栏零回归；
- 硬门：`npm test` **480 / 480 / 0 / 0 exit 0**；`npx vue-tsc --noEmit` 的 19 个 `TS2339` **全部出自第三方在途文件 `asset-inventory.vue`**，A/B 对照（抽掉本次 `padding-right` 再跑）错误集**逐字相同** ⇒ 本改动新增 **0** 错误；
- **现象/严重度更正（必写）**：D1 修前真实点击**不是「无反应」**，而是**误触图例栏第三个按钮「🗺️迁徙地图」并 `navigateTo` 跳页**（实测 `location.hash` 由 `#/pages/hall/index?tree_id=ji_23395_01` 变为 `#/pages/special/migration-map/index?tree_id=ji_23395_01`，`.layout-switcher` 随之消失）⇒ 登记进现象（影响面比原记录更大）；
- 未覆盖：`/z/` 世本与 `/z/<祖谱码>` 祖谱两页未回归（本次任务禁止改动那些页面）；真源开工/收尾 md5 逐行比对未执行（全程只走 GET 读 + 数据根指向 /tmp 副本）。

---

## 12. 家谱 tab 双 tab 口径（**追加 · 2026-09-25 · 只追加 · §1–§11 既有行原文一律保留**）

> **性质（只追加）**：本节为**纯追加章节** —— **§1–§11 既有行原文一律保留、一字不改、一行不删**；凡被取代处，**只在本节新写的行里标「已被取代」并写明由本册哪一节取代**（`AGENTS.md` §0-4 / §0-5 的追加体例；本条同样适用于本节与 §11-6 / §11-7 的关系）。
> **来源**：Kevin 已裁口径 **2026-09-25**（Zang 裁定 v1），逐条展开见**新建册** `docs/task-center.spec.md` **§1（T-1 – T-8）**；本节只做**首页册侧的取代登记与指向**，**不重写**任务中心域的口径。
> **数值 / 字面纪律**：本节**不写任何实现现状读数**（行数 / 测试条数 / 构建字节一律不写）；凡引用文件、组件、类名、路由，**一律以对应命令（`grep` / 源码现证）的实际输出为准**；**奖励数量一律不写**（唯一真源 = `docs/friend-domain.spec.md`）。

### 12-1 取代声明（**§11-2 第 1 条 ⇒ 已被取代**）

| # | §11 既有行（**原文保留，本节不回改**） | 处置 |
|---|---|---|
| 1 | **§11-2 第 1 条**：家谱 tab「已登录且已绑定 ⇒ **整页只渲染同一份 `TreeHall`**」 | **已被取代** —— 由 **`docs/task-center.spec.md` §3（T-1）** 取代为：**页面 = 顶部双 tab 切换器 + 两个面板**（tab1「我的家谱」= 现有 `TreeHall`；tab2「领取今日奖励」= 任务列表）。**「整页只渲染家族树」的表现不再成立**，但**「单真源、禁止复制第二份模板」的纪律继续有效**（见 §12-3） |
| 2 | **§11-3**（家谱 tab 页面组成 · 本批移除项与保留的空态两组） | **继续有效**；**但**：因 **T-2**，家谱 tab 的**空态分支「不显示切换器」**成为附加口径（**两态文案逐字不变**）；因 **T-6**，**「我的家谱」tab 内新增好友入口格 —— 仅此一格**（**不得**把 §11-3 已移除的 **hero 卡 / 身份节点卡 / 导航格 / 收录人物统计栏** 整组带回） |
| 3 | **§11-4 / §11-5 / §11-6 / §11-7** | **全部有效、逐字不变**（本批与它们**无交集**：D1 修复的几何与家谱 tab 的页面结构是两件事） |

### 12-2 新口径（逐条 · 唯一真源 = `docs/task-center.spec.md` §3）

1. **已登录且已绑定** ⇒ 页面 = **顶部双 tab 切换器 + 两个面板**；**默认 tab = 「我的家谱」**（进页面先看到家族树内容，**行为与改造前一致**）。
2. **tab1「我的家谱」的面板 = 现有 `TreeHall` 组件原件**（`frontend/src/components/tree-hall/tree-hall.vue`）—— **仍不得复制第二份模板**；prop **仍为** `:tree-id="boundTreeId"` 与 `:sync-nav-title="false"`；`:key="boundTreeId"` 与 `onShow` 重取锚点的既有重建行为**逐条不变**。
3. **tab2「领取今日奖励」的面板 = 任务列表**（3 条每日任务：签到 / 邀请新用户注册 / 平台写操作）。
4. **导航栏标题保持「我的家谱」** —— **`pages.json` 一字不改**（沿用 `sync-nav-title=false`）。
5. **切换器不引 `t-tabs`**；**样式参考实现** = `frontend/src/pages/index/index.vue` 的 `.sort-bar` / `.tab-switch` / `.tab-btn` / `.tab-text`（CSS 块）与 `listTab` ref（同一处先例见本册 §10-1 与 `pages/index/index.vue` 模板注释「浮动按钮切换，无 t-tabs」）；**状态变量字面以实现落地为准**（语义 = 一个二值状态决定渲染哪一面板，**不新增第二套状态**）。
6. **切换器不是 tabBar 的 tab**：**tabBar 结构不动、`pages.json` 的 `tabBar` 段一字不动**（与 §11-1 的易混澄清同一条纪律）。
7. **未登录 / 已登录未绑定** ⇒ **保持现有两组空态**（**逐字不变**）、**不显示切换器**（奖励需登录）。

- **一句话可改**：改 `docs/task-center.spec.md` **§3-1 表**「tab 名 / 面板」两格一处（本节与之一致，随之同步）。

### 12-3 不受影响项（**逐条确认**）

| 项 | 处置 |
|---|---|
| **单真源、禁止复制模板**（§11-2 第 1 条同行后半句 / 第 2 条） | **继续有效、逐字不变** —— tab1 面板仍是**同一份** `tree-hall.vue`；`pages/hall/index.vue` 仍为**薄壳**；两处「单真源，禁止复制模板」注释**逐字保留** |
| **组件内不得使用页面生命周期**（§11-2 第 2 条） | **继续有效** —— 切换器的状态与任务取数**都只留在页面**（`pages/family/index.vue`），**不写进组件** |
| **prop 契约**（§11-2 第 3 条） | **继续有效、逐字不变** |
| **导航栏标题口径**（§11-2 第 4 条） | **继续有效**；`pages.json` **一字未改** |
| **换树 = 整棵重建**（§11-2 第 5 条） | **继续有效** |
| **两组空态文案**（§11-3 尾条） | **逐字不变**（附加口径 = 不显示切换器，见 §12-2 第 7 条） |
| **§11-4 的两条取代指向**（`docs/geo-origin.spec.md` §7-5 D3 / `docs/person-places.spec.md` D3 在家谱页上**不再适用**） | **继续有效、逐字不变** —— 本批**没有**把 hero 卡（含**发源地**）带回，故那两条取代关系**不受本节影响** |
| **§11-5 / §11-6 / §11-7**（验收与部署口径 / D1 缺陷登记 / D1 修法追记） | **全部有效、逐字不变**；§11-5 中「家谱 tab 与直开 `/pages/hall/index?tree_id=…` 的家族树内容**逐字 / 逐项一致**」的判据**仍是**本批前后端交付的判据之一（落在 tab1 面板上比对） |

### 12-4 与其它册的登记关系（**只指向**）

- **任务中心域**（双 tab 展开 / 三条任务 / 手动领取 / 三态 / 池快照 = 领取时刻 / 好友入口格 / 批次边界）：唯一真源 = **`docs/task-center.spec.md`**（本册**不重写**）。
- **好友域与入口的取代**：入口位置与任务集、邀请链路、子包口径一律以 **`docs/friend-domain.spec.md` §17-2 / §17-3 / §17-4 / §17-5 / §17-6** 为准（**该册本轮未改一字**）。
- **部署登记**：本批上云 / 重打动作由 **`docs/PENDING_DEPLOY.md`** 承接（**本册不预填读数**）。

### 12-5 本节未做项（**如实登记**）

- **未改任何代码**、**未改 `pages.json`**、**未改 `AGENTS.md` / `README.md`**、**未改 `migrate-output/` 与 `config/`**、**未 commit / 未 push**、**未部署 / 未打包**、**未跑测试 / 未跑构建**。
- **本节为纯追加**（该文件**删除列 = 0**）；**未回改 §1–§11 任何历史行**；**未写任何实现现状读数**。
- **本节不含任何验收结论**（**不写「已通过」**）：上列口径的**实测结论待质检（Neng）收口后回写**。
