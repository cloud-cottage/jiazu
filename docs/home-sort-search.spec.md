...
- **`maternal_succession` 参数类型口径（只认 JSON 布尔）**：`POST /admin/add-child` 的该参数**只接受 JSON 布尔 `true`**；字符串 `"true"` / 数字 `1` 属**类型不符 ⇒ 同样 400 拒绝**（与第 2 / 3 条同一条拒绝语义，**不是**静默忽略、**不是**按真值强制转换）。前端已做布尔化提交，故本约束**对现有交互无实际外溢**。

**三、边界与不变项**

- **存量数据不在本轮修正**：本轮只**加写路径守卫**，不回溯改已有节点的归属与层级。
- **人数仍 = §10-6 口径**：家族树 = `people` − 被排除项；世本 / 祖谱 = `people` 数（**不套用**家族树排除规则）。
- **规则 2 的真源现状（登记 · 非缺陷）**：真源 **17 棵树**里**几乎没有「非本族男性真节点」**——非本族男性**全部是跨树镜像** ⇒ 被 `isMirrorNode` 放行，故规则 2 的拒绝路径**目前只由单测夹具覆盖**；**真源当前零正例、零误伤**（无回归样本，不等于规则 2 未生效）。将来真源出现**外姓女婿等非本族男性真节点**时，规则 2 才会在真源上真拦。
