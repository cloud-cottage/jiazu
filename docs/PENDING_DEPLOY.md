...
## §40 兰帖残页【手动合成】批次（**云函数重打包 + H5 / 小程序两产物重打** · **无集合变更 · 无数据修正（存量不迁移）** · 2026-09-26 · **只追加 · 不改 §0–§39 任何行**）（Jing 制度员）

> **性质（只追加）**：本节为**纯追加章节**（**§0–§39 既有行原文一律保留、一字不改、一行不删**；被更正之处**只在新行里写明取代关系**，承 `AGENTS.md` §0-4）。**落盘方 = Jing（制度员）**；**本阶段不上云、只登记**（**未打包 / 未部署 / 未上传 / 未重传**）。
> **口径真源**：`docs/friend-domain.spec.md` **§25**（**主册**：取消【满 100 自动合成】+ 改用户【手动合成】）· `docs/economy.spec.md` **§21-8**（数值 `SCROLL_FRAGMENT_CAP` 99 → 999 + 行囊展示 = 每 999 片 1 格）· `docs/economy-ops.spec.md` **§14-14** + **§14-13 关账行**；**登记侧** = `AGENTS.md` **勘误追加区勘误 11** + **§7 追加行**。**本节只写动作与判据、不预填任何读数**（测试条数 / 构建字节 / 产物 md5 / 路由条数一律以对应命令的实际输出为准）。
> **裁定来源**：Kevin **2026-09-26 当面裁定**（① **取消【兰帖残页】满 100 自动合成**；② **改用户【手动合成】**）+ **Zang 裁决**（**账本层 vs 路由层严格度差异只登记、两侧均不改**）。

### 40-0 批次定性与动作面（**3 项必登**）

| # | 动作面 | 触发依据 | 本轮定性 |
|---|---|---|---|
| **①** | **云函数 `compat-api` 重打包 + `tcb fn deploy`** | 本批改 `cloudfunctions/compat-api/lib/economy-ledger.js`（`SCROLL_FRAGMENT_CAP` **99 → 999**；`addScrollFragments` 改**纯累加**；新增导出 `synthesizeScroll`）· `lib/economy-ops.js`（`grantAssets` 残页正向分支**不再顺带合成**）· `index.js`（新增路由 `POST /assets/scroll/synthesize`）⇒ **不重打包 = 云端仍走「满 100 自动合成」且新路由 404** | **必登**（见 §40-1） |
| **②** | **前端 H5 + 小程序两产物重打** | 本批改 `frontend/src/business/inventory.ts`（残页 **每 999 片占 1 格** + `SCROLL_FRAGMENT_PIECES_PER_CELL = 999`）· `frontend/src/components/asset-inventory/`（残页格提示层新增【合成】按钮）· `frontend/src/business/asset-text.ts`（`SHORTAGE_UNIT_BY_KEY` 补单数键 `scroll`，commit `5f927f1`）⇒ **不重打 = 用户面看不到【合成】入口、行囊仍按旧「1 格」显示** | **必登**（见 §40-2） |
| ③ | 集合 / 云端数据 | 本批**不新增集合**（残页仍落**既有** `jiazu_assets`）· **无数据修正（存量不迁移）** | **本轮无**（见 §40-3） |

- **顺序纪律**：按 **① → ②** 执行（**先重打包云函数 → 再前端重打**；承 §35-2 / §38-0）。
- **⚠️ 前置门槛**：**`npm test` 未全绿前不得打包 / 部署 / 重传**（承 §29-6 第 1 条 / §36-8-4 / §37-0 / §38-0）；**另需 `cd frontend && npm run type-check` = `EXIT 0`**。**注册差量必核**（**未注册 = 假绿**；`scripts.test` 注册项数与磁盘 `cloudfunctions/compat-api/lib/*.test.js` 个数**必须相等** —— **本批现证 = 35 = 35**，**执行时自行重算**，承 §14-4 / §16-1 / §38-0 体例）。
- **本节不登记实现状态**（不进「已实现 / 已通过」口径）：代码实现归**另单**（Kong）；本节**只承接上云动作与判据**。
- **⚠️ 旧机理残留 = 本批判负项（硬）**：本批**取消自动合成** ⇒ 打包后**旧「满 100 自动合成」分支在产物内应为 0 命中**（判据见 §40-1 判据 3）。

### 40-1 必登 ①：云函数 `compat-api` 重打包 + `tcb fn deploy`

- **打包命令（沿用既有流程，逐字同 §1 / §35-1 / §38-1）**：
  - `npx esbuild cloudfunctions/compat-api/index.js --bundle --platform=node --format=cjs --external:@cloudbase/node-sdk --outfile=cloudfunctions/deploy/compat-api/index.js`
  - 再 `tcb fn deploy compat-api -e <envId>`（**envId 以 `cloudbaserc.json` 的现证值为准**；承 §38-1 的「旧行 `…d087e` vs 现证 `…d087c`」登记 —— **执行时按文件取材，不改旧行**）。
- **判据形态（**只登记形态、不登记数值** —— 逐条执行时贴实际输出）**：

  | # | 判据 | 命令形态 | 通过条件 |
  |---|---|---|---|
  | 1 | 新路由字面已进产物 | `grep -c "/assets/scroll/synthesize" cloudfunctions/deploy/compat-api/index.js` | **≥ 1**（**0 命中 = 未打包或漏改 ⇒ 判负**） |
  | 2 | 手动合成文案已进产物 | `grep -F -c "手动合成" …`（**中文文案须 `grep -F`**） | **≥ 1** |
  | 3 | **旧自动合成分支已消失（本批核心判据）** | 对产物 `grep -n` 旧分支标志（残页自动合成写入点 / `synthesized` 相关分支） | **旧分支 0 命中**（**残留即判负** —— 与判据 1 / 2 互为反证） |
  | 4 | 上限常量已取 999 | `grep -c "999" …` + 对 `SCROLL_FRAGMENT_CAP` 相关段 `grep -n` | **含 `999`**（**以命令输出为准，不预填**） |
  | 5 | 发放不再合成（分支字面） | 对 `grantAssets` 相关段 `grep -n`（残页正向累加分支） | **含「纯累加 / 不自动合成」语义分支**（**本批不新增作废字面**） |
  | 6 | 既有路由仍在（未被本批破坏） | 对 `/assets/scroll/decompose` · `/assets/summary` · `/admin/assets/grant` 各 `grep -c` | **各 ≥ 1** |

- **中文判据注**：**`esbuild` 默认 `--charset=ascii`，产物内中文为 `\uXXXX` 转义** ⇒ **凡以中文文案做判据须用 `grep -F` 定长匹配**（体例承 §9-2 尾注 / §35-1 / §38-1）。
- **重打包后本轮产物 md5 与 §32–§39 的旧产物 md5 必然不同** ⇒ **以命令实际输出为准**（**旧行一律原文保留**）。

### 40-2 必登 ②：前端 **H5 + 小程序**两产物重打（随 ① 之后）

- **H5**：`cd frontend && npm run build:h5`（**构建前注入 `VITE_API_BASE` = 云函数 HTTP 域名**，取值口径同 §1 / §7-4）⇒ `tcb hosting deploy frontend/dist/build/h5 -e <envId>`。
- **小程序**：`cd frontend && npm run build:mp-weixin` ⇒ **用微信开发者工具打开对应产物目录并上传**（上传动作与版本号由执行方按既有惯例填写）。
- **判据形态（**只登记形态**）**：

  | # | 判据 | 命令 / 动作形态 | 通过条件 |
  |---|---|---|---|
  | 1 | 残页格数常量已进 H5 产物 | 对 `frontend/dist/build/h5/**` `grep -c "999"`（+ `SCROLL_FRAGMENT_PIECES_PER_CELL` 相关段） | **≥ 1**（**0 命中 = 未重打或产物过期 ⇒ 判负**） |
  | 2 | 【合成】入口字面已进产物 | 对同一产物树 `grep -F -c "手动合成"` / 对残页格提示层动作面 `grep -n` | **≥ 1**（**以实际产物路径为准**） |
  | 3 | 小程序产物同步 | 对 `frontend/dist/build/mp-weixin/**` 重跑判据 1 / 2 | **同通过条件** |
  | 4 | 行囊可见性 | 部署后打开「我的 → 行囊」⇒ **期望**：**残页格**提示层内可见【合成】按钮（与**兰帖格【分解】**对偶）；残页 **≥ 100 片**时可点 | **可见且可点**（**以实际页面为准**） |
  | 5 | 占用格数显示 | 残页片数 **≥ 1000** 的账号 ⇒ **期望**：**每 999 片占 1 格**（整格角标 **999** / 余数格角标 = **余数**） | **与 `docs/economy.spec.md` §21-8 逐字一致** |
  | 6 | 字节数 / md5 / 文件数 | `wc -c` / `md5 -q` / `find` | **一律以命令实际输出为准**（**不预填**） |

### 40-3 云端动作：**无新增集合 / 无数据修正（存量不迁移）**

- **无集合变更**：本批**不新增集合** —— 残页仍是**既有** `jiazu_assets` 的 `scroll_fragments` 标量 / `scrolls[]` 批次 ⇒ **不改 `scripts/upload-migrated-to-cloudbase.mjs` 的 `COLLECTIONS`**（**现证基线以该文件为准，执行时自行重算**）。
- **无数据修正（硬）**：**历史 `source: 'scroll_synth'` 批次与历史流水一律保留、不迁移、不改写、不删除**；**脏数据不清理** —— 账上已有的残页标量**原样留在字段里**，等用户手动合成（**上线不触发任何批量写盘**）。
- **与已知部署阻塞项（§7-7）的关系**：本批**不触碰 `jiazu_assets` 的多实例并发形态**（**不重构为「每手机号一文档 + 乐观锁」**）⇒ **既不影响、也不解除** §7-7 欠债（**上云决策仍须显式记入该欠债**，口径同 §35-5 第 4 条 / §36-3 / §38-3）。

### 40-4 部署后冒烟验证（**按序做** · 每步 = 判据 + 期望；⚠️ 结论由执行 / 质检收口后回写，**本清单不预填**）

1. **合成成功（正向）**：残页 **≥ 100 片**的账号 ⇒ 行囊残页格【合成】⇒ **期望**：**`200`** + `{ok:true, message:'兰帖已合成', data:{…}}`；**残页 −100 片**、**兰帖 +1 张**（`delta = {scroll_fragments:-100, scrolls:+100}`，**两键均以片计**）；该用户 `txs` 新增**一条** `type='scroll_synth'`，`desc` **逐字** = `手动合成 1 张兰帖（消耗 100 片兰帖残页）`；**未写 `edit_fee`、未扣竹片**、**每张批次 `expires_at = null`**。
2. **余数保留**：残页 **150 片** ⇒ 合成 1 张 ⇒ **期望**：残页余 **50 片**（**绝不出小数张**）；**再合一次 ⇒ `409` 不足**（**不部分合成**）。
3. **不足 409（负向 · 出参形状硬）**：残页 **< 100 片** ⇒ 【合成】⇒ **期望**：**`409`**，出参 = `{ok:false, error:{code:'ASSET_INSUFFICIENT', status:409, message:'资产不足，需 100 片兰帖残页，当前 M 片'}}` —— **⚠️ 该体只含 `code` / `status` / `message`**（**不带 `need` / `current` / `unit`**；**与 `/assets/scroll/decompose` 同形**）；**零写入**（标量 / 批次 / 流水全不动）。
4. **非法 `count` ⇒ `400`（API 直调判据）**：直调 API 传 `count = 0` / `-1` / `1.5` / `abc` ⇒ **期望**：**`400`**，`code = 'INVALID_COUNT'`，文案**逐字** = `count 必须为正整数（1 张 = 100 片）`（**用户面经 UI 不可达** ⇒ 本条**只作 API 直调判据**）。
5. **发放不合成（硬 · 本批核心判据）**：`POST /admin/assets/grant` 发 `scroll_fragments` 使账上残页 **≥ 100**（如 `+100`）⇒ **期望**：**`200`**，且**发放后残页原样保留**（**不自动生成兰帖**、`scrolls` **不变**、**无 `scroll_synth` 流水**）；**快照 `scroll_fragments` = 发放后值**（**未取余**）。
6. **cap = 999（快照 + 展示）**：`GET /admin/assets/user?phone=…` ⇒ **期望**：`scroll_fragment_cap = 999`；残页 **≥ 1000 片**的账号 ⇒ 行囊内**每 999 片占 1 格**（占格数 = `ceil(片数/999)`；整格角标 999 / 余数格 = 余数）。
7. **分解不回归（对偶项）**：残页 **≥ 100 片**时分解 1 张兰帖 ⇒ **期望**：返 **99 片**残页（**留 1 片**）；**分解后残页为 99 片 ⇒ 不自动合成**（**R-7 禁止写法继续有效**）；`desc` 与既有一致。
8. **【合成】与【分解】对偶可见性**：行囊**兰帖格**提示层 = 【分解】、**残页格**提示层 = 【合成】⇒ **期望**：两按钮**各自可点、互不串位**；提示层**定位 / 命中坐标 / 二次确认层**口径**不变**（承 `docs/economy.spec.md` §14-9）。
9. **权限与鉴权（不变项回归）**：未登录 ⇒ **401**（`FRIEND_UNAUTHORIZED`，**不降级 guest**）；后台 `grant` 权限口径不变（`tree_steward` / `user` ⇒ `403`）。
10. **真源零写入核对（冒烟窗口）**：`find migrate-output config -newermt "<冒烟开始时刻>" -type f` ⇒ **期望为空**；若因读侧 `sweep` 只动 `mtime`，**须逐文件记录 `mtime` + `md5` 并登记**（口径承 `AGENTS.md` §2.1 / §32-3 / §38-4 第 8 条）。
11. **未裁项不判负（硬）**：本批**不裁**任何【待裁】/【未决】项；`docs/friend-domain.spec.md` **§25-9** 所列关联项（注销六类 / 「返还 N 个残页」单位词 / 运营六类面）**一律不因本批失效**。

### 40-5 **真源零写入声明行（本单 · 逐字）**

- **本单 = 制度员文档落盘单**（落点 = `docs/friend-domain.spec.md` **§25** / `docs/economy.spec.md` **§21-8** / `docs/economy-ops.spec.md` **§14-13 关账行 + §14-14** / 本节 / `AGENTS.md` **§7 追加行 + 勘误追加区勘误 11**）⇒ **本单对真源零写入**（`migrate-output/` 与 `config/` **未改 / 未移 / 未删**，全程只读）。
- **本单未上云、未跑命令**：**未打包 / 未部署 / 未上传 / 未重传 / 未建集合**、**未跑测试 / 未跑构建**（**读数引用 = Kong 实现单 / Neng e2e**，登记落点 = `AGENTS.md` **§7 追加行**；**本单不自报读数**）。
- **口径（承 `AGENTS.md` §2.1）**：**后续本机验证优先用 `/tmp` 副本栈**（`COMPAT_OUT_DIR` / `COMPAT_META_FILE` 钩子），**避免无谓 touch 真源**；确需对真源跑时**先备份、后登记前后 md5**。
- **与 §38-5 / §39-⑧ 的关系**：那两处是**前几批**的真源登记（**旧行原文保留**）；**本节不重复登记第三方写入**；**若执行阶段发现真源被动，按 §36-5 体例另起新行登记，并标「归属 = 待 Kevin 确认」**（本单**无现场第三方写入读数**）。

### 40-6 明确**不需要**上云的东西

- **`cloudfunctions/deploy/**` 的「编辑」**：该目录是**打包产物、不是编辑对象**（`AGENTS.md` §2.1 / §35-7 / §36-6 / §37-3 / §38-6）—— 本节只**登记重打包动作**。
- **`scripts/*` 一次性脚本 / `shell-scripts/**` / `/tmp/jiazu-*` / `backups/**` / `~/jiazu-backups/**`**：离线 / 临时产物，**不参与打包、不同步云端**（`AGENTS.md` §2.4）。
- **`docs/**`**：规格与台账**不上云**（本节即纯台账追加）。
- **`auth-server/**`**：已退役 / 仅排查用链路，**不进云**（本册 §6）。
...
- **未打包 / 未部署 / 未上传 / 未重传 / 未建集合**（**本阶段不上云、只登记**）；**未跑测试 / 未跑构建**；**本节不写任何实现读数**（测试条数 / 构建字节 / 产物 md5 / 路由条数一律**以对应命令的实际输出为准**）。
- **未改任何代码 / 配置 / `migrate-output/` / `config/`**（**真源只读**）；**未改 §0–§39 任何历史行**（**纯追加**）；**未碰任何 `.qa.md`**（`AGENTS.md` §0-5）。
- **未裁任何【待裁】/【未决】项**。
- **一句话可改**：改本节 §40-0 表任一行一处（须同步 §40-1 / §40-2 判据与 §40-4 冒烟清单）。
