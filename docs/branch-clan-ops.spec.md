...
## 16. 始祖真源反转（v2）对本册的**推广与放宽**（Zang 裁定 2026-09-20 · **只追加 · 不改 §1–§15 历史行**）

> **性质**：本节由 **Jing（制度员）** 追加（2026-09-20）—— **只写文档**（未改代码、未写真源、未打包 / 未部署）。§1–§15 历史行**原文保留**；被推广 / 放宽的旧行**不回改正文**，一律以本节的「已作废 / 已取代」标注表落地（同 `AGENTS.md` §0 第 4 条）。
> **裁定来源**：Zang 终审 v1「始祖真源反转」**变体 A** = 始祖**真身在家族树**；宗谱 / 世本各持**只读镜像**；**世系链镜像方向不变**。完整口径 = `docs/founder-attach.spec.md` **§9**；本册相关影响 = `docs/clan-tree.spec.md` **§11**；部署登记 = `docs/PENDING_DEPLOY.md` **§29**。
> **实现状态（实测 2026-09-20 18:00 CST）**：本节的推广 / 放宽**已在工作区落盘（未提交）** —— `lib/branch-clan-ops.js`（`8 +` / `14 -`）、`lib/founder-attach.js`（`396 +` / `91 -`，R4 主体）、`lib/tree-write.js`（`18 +` / `5 -`）、`lib/founder-attach.test.js`（用例**在途**）、`scripts/migrate-founder-inversion-2026-09.mjs`（**新增未入库** = R7）、`README.md`（脚本用法）；**`cloudfunctions/compat-api/index.js` 本批未改**、**云函数产物未重打包**。测试基线为红（**454 / 447 / 7**，见 §16-5）⇒ **不得**据本节认为已验证 / 已部署。落地证据与锚点见 `docs/founder-attach.spec.md` **§9-10**。

### 16-0 旧行处置（**标注 · 不重写正文**）

| 旧位置 | 旧口径（**原文保留**） | v2 处置 |
|---|---|---|
| **§5-3**（`external_*` 字段写法） | 「**立支写两组镜像**」——新树始祖镜像 1 个 + **宗谱登记镜像 2 个**（原树登记 / 新树登记） | **推广**：宗谱登记镜像**不再是立支专有**，而是**所有认祖关系的通用口径**（R4，见 §16-1）；立支仍写 2 条登记（原树 + 新树），**形态不变** |
| **§6-1-6**（宗谱侧与支系入口列表） | 「**立支后**…新增一条判定：宗谱自有段里 `external_link_type='founder'` 且 `external_mirror='true'` 的登记镜像，按其 `external_tree` 计入支系入口」 | **推广**：该判定**升格为通用读侧口径**（不止立支后，任何认祖关系成立后同样成立）→ §6-1-6 的**判据文字不变、适用范围放宽** |
| **§3-8**（汇宗的迁移语义 · 源树始祖判定，状态 = **已拍板**） | 「只要求当前始祖是**指向本层之上的镜像** —— `external_mirror='true'` + `external_link_type='founder'` + `external_tree` 非空且 **≠ 源树自身**」 | **由本裁定放宽**（R5，见 §16-2）：**「镜像（任一方向）或真身」**均可汇宗 —— 旧行原文保留，放宽以本节生效 |
| §4 对照表「作用对象」/「前置」两行 | 「始祖节点（**指向本层之上的镜像**，§3-8）」；「① 源树 `kind='family'` 且当前始祖是**指向本层之上的镜像**」 | **由 R5 放宽同步**（同上，只登记，不改表） |
| §6-2-2 表「源树当前始祖**是「指向本层之上的镜像」**」行 | 同 §3-8 的四项判定 + 文案「该家族树当前始祖不是上层镜像，无法汇宗」 | **由 R5 放宽**：判定与**文案**是否随改由 Kong 按实装决定；本册**不预设新文案字面** |
| §5-2「`founder_handle`」行（立支 · 原树列） | 「旧始祖镜像 handle → **N.handle**（真身）」 | **不再总是成立**：v2 下原树始祖**本就可能是真身**（立支时「原树始祖为真身则保持真身」，见 §16-2）；该行只在「原树始祖当时为镜像」时按原口径 |
| §13-20（**祖谱登记镜像的副作用** · 待裁决） | 三条副作用：自有段统计不含它 / `clanInfo.mirrors` 会列出它 / 重认祖会清掉它 | **影响面扩大（登记 · 不改 §13-20）**：登记镜像由「立支专有」变「通用」⇒ 上述三条副作用的作用面同步扩大；**仍未裁决**，本节不代拍板 |
| §13-22（**立支后原树残留的旧始祖镜像** · 待裁决） | 「立支后原树里仍存在 `external_link_type='founder'` + `external_mirror='true'` + `external_tree` = 本祖谱的旧始祖镜像节点」 | **受影响（登记 · 不改 §13-22）**：v2 下原树始祖为**真身**时**不存在**该残留镜像；为**镜像**时残留现象与两条推导路径并存的问题**照旧**。仍未裁决，本节不代拍板 |

### 16-1 R4 宗谱登记镜像 = 所有认祖关系的**通用口径**（§5-3 / §6-1-6 的推广）

- **通用形态**：宗谱**自有段**持 **1 个**指向**家族树始祖（真身）**的只读镜像 —— `external_link_type='founder'`、`external_mirror='true'`、`external_tree='<家族树 tree_id>'`；**宗谱内只读**；**同一家族树至多 1 条**（一树一登记）；**宗谱 `tree-meta.founder_handle` 指向它**。
- **与立支的关系**：立支仍写 **2 条**（原树登记 + 新树登记，`lib/branch-clan-ops.js:181` `planBranchRegistrations`，注释 `:175-180` 逐字「原树登记：`external_tree = 原树`、`external_person_handle = N.handle` / 新树登记：`external_tree = 新树`、`external_person_handle = 新树始祖镜像 handle` / 两条都是 `external_link_type='founder'` + `external_mirror='true'`」）—— 形态不变，语义由「立支专有」升格为「通用口径的 2 个实例」。
- **读侧**：支系入口列表推导（§6-1-6）适用面放宽为「任何成立了的认祖关系」，判据文字不变。
- **实测现状（未落盘）**：真源当前**没有**任何「指向家族树始祖真身」的宗谱登记镜像 —— `ji_23395` / `gu_39038` 的 `tree-meta.founder_handle` 分别指向宗谱自有段的**真身节点**（`3c95530f8bd4f84dc0b87edc` = 季花 I000163 / `5ae4c6e505c90d290f71f66b` = 顾清学 I000139）；「2 条家族树始祖镜像」（`ji_23395_01` I000209 季花 / `gu_39038_01` I000143 顾清学）当前是**镜像**（指向宗谱），与 v2 相反 ⇒ 需**存量就地反转**（见 §16-3）。

### 16-2 R5 立支 / 汇宗的**连带口径**

1. **立支新树始祖**：仍为 **N 的镜像**（**同层指向** = 指向原树里的真身 N，`external_tree='<原树 tree_id>'`、`external_mirror='true'`、只读；`lib/branch-clan-ops.js:221` 注释「新树始祖镜像节点（§5-3：展示副本 + 指向 N 真身）」、`:231` 写 `external_link_type: FOUNDER_LINK_TYPE`）—— **本裁定不动它**（同层镜像不属于「上层镜像」）。
2. **立支原树始祖**：**原树始祖为真身则保持真身**（不再改成镜像、不再被覆盖身份字段）；为镜像时按既有口径处理（§5-2 / §13-22 的残留现象照旧登记、未裁决）。
3. **汇宗前置（放宽 · 取代 §3-8 与「已拍板」行的收紧表述）**：源树始祖从「**上层**镜像」放宽为「**镜像（任一方向）或真身**」。
   - **镜像** → 按现状处理：**丢弃**（不随迁；§3-8 原口径「源树的始祖镜像节点本身不随迁、直接丢弃」不变）。
   - **真身** → **随真实节点整体迁入**（与其它真实节点同路径：`handle` / `gramps_id` 不变、详情随迁并显式改 `_id`）。
4. **落盘实测（2026-09-20 18:00 CST）**：`convergeClan` 的前置**已按 R5 放宽** —— `lib/branch-clan-ops.js:818-824`（注释 `:818-821` 逐字「源树当前始祖：**镜像（任一方向）或真身**都放行（R5 放宽口径；§3-8 / §6-2-2）」），判定收敛为「**能识别出始祖节点**」（`:822-824`），`throw` **`:824`** = 新字面 `该家族树当前没有始祖节点，无法汇宗`；**旧字面 `该家族树当前始祖不是上层镜像，无法汇宗` 在本文件中已 0 命中**（实测 `grep -c` = **0**）。⚠️ **测试端尚未跟改**：`lib/branch-clan-ops.test.js` 的 `not ok 52`（「§6-2-2 拒绝 · 源树始祖是真人节点（非镜像）→ 400『不是上层镜像』」）等用例仍按旧口径断言 ⇒ **本册不据此判定实现有误**，跟随状态以 Kong 收尾为准。

### 16-3 存量数据（季 / 顾两条链）**就地反转**（R7 · 动作面登记）

- **口径**：`ji_23395` / `gu_39038` 两条「宗谱自有段真身 + 家族树始祖镜像」的链**就地反转**为 v2 形态：**家族树始祖 = 真身**、**宗谱侧 = 只读登记镜像**；**宗谱顶端镜像段方向不变**（`ji_23395` I000162 季行父 / `gu_39038` I000138 姒期视 → `zhonghua`）。
- **要求**：**先备份**；**详情随迁并显式改 `_id`**（`<宗谱 tree_id>:<handle>` → `<家族树 tree_id>:<家族树始祖 handle>`）；**登记前后 md5**。
- **旧详情键（候选清单 · 以 Kong 实装 report 为准）**：本节实测**存在**的旧键 = `ji_23395:3c95530f8bd4f84dc0b87edc`（季花，273 B）与 `gu_39038:5ae4c6e505c90d290f71f66b`（顾清学，276 B）；对应的家族树侧现有键 = `ji_23395_01:10400594c54f5203f61bf4fa4b20`（季花，333 B）与 `gu_39038_01:103f95b87b5a464242933ee319d5`（顾清学，336 B）。**最终键对以实装为准**，清理顺序一律「**先重传、再删旧键**」（`docs/PENDING_DEPLOY.md` §29-3）。
- **本节边界**：**只登记**，不执行迁移、不预判实现细节（是否搬节点 / 是否改 `founder_handle` 指向 = Kong 落地）。

### 16-4 代码锚点（**本节实测行号 · 逐字取自实现**）

> **本轮校正（Jing · 2026-09-20 **18:28** CST 逐行复核）**：整表行号已按 `sed -n '<n>p' cloudfunctions/compat-api/lib/branch-clan-ops.js` 重新实测；**两处漂移已改**：`applyConvergeMove` **`:381` → `:380`**、`clanAffiliationOf` **`:760` → `:763`**（其余各锚点复核后不变）。
> ⚠️ **代码仍在被 Kong 收尾修改中**（该文件 mtime 实测 **2026-09-20 17:56:23**）⇒ **本表行号一律带时点读**；下次改动后需再复核。

| 锚点 | 位置 | 说明 |
|---|---|---|
| `planBranchRegistrations` | `lib/branch-clan-ops.js:181` | 立支两条宗谱登记（R4 的现有实例） |
| 新树始祖镜像 | `lib/branch-clan-ops.js:221`（注释）/ `:231` | 同层指向 N 真身（R5 第 1 条） |
| `planConvergeMove` / `applyConvergeMove` | `lib/branch-clan-ops.js:332` / **`:380`**（旧记 `:381` 已作废 · 实测 `:380` 是 `export function applyConvergeMove`，`:381` 是其体内首行调用） | 汇宗迁移集合与落点 |
| `establishBranch` | `lib/branch-clan-ops.js:506` | 立支编排 |
| `clanAffiliationOf` | `lib/branch-clan-ops.js:763`（旧记 `:760` 已作废 · 实测 `:760` 是上一函数的收尾 `}`；函数定义在 `:763`）（调用 `:546`） | 宗谱归属判定（§3-6） |
| **汇宗前置（R5 落盘点）** | `lib/branch-clan-ops.js:818-824` | 注释 `:818-821`（逐字「源树当前始祖：**镜像（任一方向）或真身**都放行（R5 放宽口径；§3-8 / §6-2-2）」）；判定 `:822-824`；`throw` **`:824`** = `该家族树当前没有始祖节点，无法汇宗`（**旧字面「…不是上层镜像，无法汇宗」已 0 命中**） |
| R5 立支侧 | `lib/branch-clan-ops.js:702-703` | 逐字「R5（变体 A）：原树始祖 = **N 真身**（真实节点，非镜像）——只改 tree-meta 登记，**不给 N 写任何 `external_*`、不强转镜像**」 |
| R4 登记镜像（宗谱侧） | `lib/founder-attach.js:533` / `:548` / `:556` / `:573` | `isClanRegistration` / `clanRegistrations` / `assertOneRegistrationPerFamilyTree`（重复 → 400）/ `planClanRegistrationMirror` |
| R7 迁移脚本 | `scripts/migrate-founder-inversion-2026-09.mjs` | **新增（`??` 未入库）**，用法登记在 `README.md`（dry-run 默认 / `--apply` 写前自动备份到 `~/jiazu-backups/<日期>-founder-inversion/`）；**本节实测尚未跑过**（该备份目录不存在） |
| **锚点漂移登记（如实）** | 同 §16-4 第 1 行 | §15 **#17** 记「前置段 file:line **811–825**，`throw` 在 **:825**」，本节初稿（17:44）实测 **822–829 / `:829`**，**落盘后（18:00）实测 818–824 / `:824`** ⇒ **以最新实测为准**；§15 #17 的数字行**原文保留**（不回改），漂移本身即「旧口径行会随实现移动」的例证 |

### 16-5 测试基线（本节时点实测）

- **本轮校正实测（2026-09-20 **18:22:53** CST · **取代**下方旧读数）**：`cd /Users/kevin/bistro/jiazu && npm test` → **454 tests / 447 pass / 7 fail / 0 skipped**（**exit 1** = **红**）。7 红项 = `not ok` **42 / 52 / 228 / 229 / 230 / 234 / 236**（`node --test` TAP 输出，Jing 实测）—— **失败归属**：**Kong 未完成的旧断言改写**（测试端尚未跟改本批新口径；本册 §16-2 第 4 条已实测「旧字面 `该家族树当前始祖不是上层镜像，无法汇宗` 在 `lib/branch-clan-ops.js` 中 0 命中」，而 `not ok 52` 仍按该旧口径断言 ⇒ 红的成因在测试端，**本册不据此判定实现有误**）。
  ⇒ **本册不得据此写成绿**；**待全绿后由本册追加一行**（本册**不推算、不预填**）。
- **已作废读数（原文保留）**：`npm test` **454 tests / 454 pass / 0 fail / 0 skipped**（exit 0，**2026-09-20** 本节初稿时点）—— 该读数**已作废 / 已陈旧**（Kong 于 17:51–17:56 在途落盘后即为红）。本册 §10-2 / §15 #13 记的 327 pass 等数字为**历史读数（原文保留）**。
- **全绿读数（**本节追加 · 2026-09-20 **18:38:20** CST 实测 · **取代上条红读数**）**：`cd /Users/kevin/bistro/jiazu && npm test` → **454 tests / 454 pass / 0 fail / 0 skipped**（**exit 0** = **全绿**）。收尾内容 = **测试端跟改本批新口径**（`lib/branch-clan-ops.test.js` `26 +` / `19 -`、`lib/founder-attach.test.js` `127 +` / `57 -`、`lib/founder-reattach.test.js` `18 +` / `9 -`，实测 18:31–18:36 落盘）⇒ 上条 **454 / 447 / 7（红）** 为**收尾前**读数、**已被取代**。**`npm test` 全绿 = 本批交付门槛，现已达标**（仍以**冻结时点的最终一次实测**为准）。
- **v1.2 读数刷新（本节追加 · 2026-09-20 **19:18:14–19:18:17** CST 实测 · **取代上条 454 基数**）**：`cd /Users/kevin/bistro/jiazu && npm test` → **464 tests / 464 pass / 0 fail / 0 skipped**（TAP 逐字 `1..464` / `# pass 464` / `# fail 0` / `# skipped 0`，**exit 0** = **全绿**）。增量成因 = **专测文件 `lib/founder-source-reversal.test.js` 落地并注册**（**44,276 B**，mtime **2026-09-20 19:04**；`scripts.test` 注册数 = **28** = 磁盘 **28**）⇒ **+10 tests**（454 → 464）。**交付门槛仍达标**。
- **A6 回退登记（裁定 D1 · 本节追加 · Jing · 2026-09-20 **19:19** CST 实测）**：`/tree/rank` **不接线 `resolveChainGen`**（`index.js` md5 **`f1f40096323de98f1f2b3d9188c2bac5`** = 与 **HEAD 字节相同**；`grep -c 'resolveChainGen' cloudfunctions/compat-api/index.js` = **0**）；**R6 唯一落点 = `lib/tree-write.js` `clanSelfGenMap`（`:772` / `:781`）保留**。⇒ 本册 §16-1 / §16-2（R4 / R5）**均不受 A6 回退影响**（回退只涉及 `/tree/rank` 的世数读数层）。逐项读数与裁定理由见 `docs/PENDING_DEPLOY.md` **§29-9「v1.2 裁定 D1 校正」**。
- 本节**未写任何测试**，仅登记读数。
...
grep -n 'planBranchRegistrations\|planConvergeMove\|establishBranch\|clanAffiliationOf\|该家族树当前始祖不是上层镜像' cloudfunctions/compat-api/lib/branch-clan-ops.js
python3 -c "import json;t=json.load(open('config/tree-meta.json'))['trees'];print({k:(v.get('kind'),v.get('founder_handle')) for k,v in t.items() if k in ('ji_23395','ji_23395_01','gu_39038','gu_39038_01')})"
...
### 16-7 **R7 真源迁移「已落盘」状态行 ＋ E1 / E2 裁定登记**（Jing 制度员 · 2026-09-20 **21:31–21:36** CST 实测 · **只追加 · 只标注取代 · §16-0～§16-6 历史行原文保留**）

> **取代标注（不改历史行）**：本节 **§16-3 第 3 条「旧详情键（候选清单 · 以 Kong 实装 report 为准）」** 与 **§16-3 末条「只登记，不执行迁移」** —— **R7 存量就地反转已于 2026-09-20 21:31:28 CST 对真源执行完毕**（`node scripts/migrate-founder-inversion-2026-09.mjs --apply` · **7 改 / 2 删** · 幂等已验证 —— 二次 `--apply` = 改动项 0 / 待写 0 / 待删 0 · `migrate-output/**` **319 → 317**）。**✅ 旧详情键已定案（不再是候选）**：**将删除的旧键 = `ji_23395:3c95530f8bd4f84dc0b87edc`（季花，删前 md5 `6eef4a753ba55a8d7e78fe40bfef5cd1`）**、**`gu_39038:5ae4c6e505c90d290f71f66b`（顾清学，删前 md5 `66938016d817ce8b12f02686a7323dba`）** —— **本地真源已删**（现盘实测 `ABSENT`，备份副本在 `~/jiazu-backups/2026-09-20-founder-inversion/deleted-source-details/`，副本 md5 与删前**一致**）；**云端删除动作未执行**，顺序仍「**先重传、再删旧键**」。**逐项事实（命令 / 7 改 md5 表 / 幂等 / 备份与 9 条回滚 / 文件数 / tracked 文件注意 / 云端动作回填）= `docs/PENDING_DEPLOY.md` §29-10**；**运维口径与 E1/E2 全文 = `docs/founder-attach.spec.md` §9-11**。

**（a）「R7 已落盘」状态行（带日期 · 实测）**

| 项 | 状态（2026-09-20 **21:31–21:36** CST 实测） |
|---|---|
| **R4 登记镜像（§16-1）** | **✅ 存量已成事实态**：`ji_23395` / `gu_39038` 宗谱自有段各持 **1 条**指向家族树始祖真身的只读登记镜像（`external_tree=ji_23395_01` / `gu_39038_01`、`external_person_handle=10400594c54f5203f61bf4fa4b20` / `103f95b87b5a464242933ee319d5`、`external_mirror='true'`）⇒ §16-1 的通用口径在存量数据上成立 |
| **R5 立支 / 汇宗（§16-2）** | **不受影响**：两条链的反转只改家族树始祖的 `external_*` 与两棵宗谱登记镜像的指针；**立支 / 汇宗判定代码本批未改**（R5 放宽口径仍在 `lib/branch-clan-ops.js:818-824`，锚点见 §16-4） |
| **R7 脚本状态** | **✅ 已执行**（此前 §16-4 表末行「**本节实测尚未跑过**（该备份目录不存在）」**已作废** —— 备份目录 `/Users/kevin/jiazu-backups/2026-09-20-founder-inversion/` 已存在且含 7 改前副本 + `md5-before.txt` + `summary.json` + `deleted-source-details/{2 + md5.txt}`；**9 条回滚命令**由脚本 stdout 打印，**备份目录内无 `.sh`**）；脚本本身**仍未入库**（`git status` = `??`） |
| **家族树侧（§16-3 反转目标）** | **✅ 已成事实态**：`ji_23395_01` / `gu_39038_01` 始祖 = **真身**（`external_*` 6 个字段**全为空串**，含 `external_founder_created_by`）；始祖详情（A2：只携带内容字段、身份键取本树现值）已就地写入（`ji_23395_01:10400594…` = `f0153ea08aa568f867d4fd96c4f52512`、`gu_39038_01:103f95b8…` = `81e7c545c89cbe0c92e958e735ed58a1`） |
| **云端 / 部署** | **❌ 未执行**：树 JSON 重传 + 旧详情键删除 + 云函数重打包 + 前端重打（四项判据同 `docs/PENDING_DEPLOY.md` §29-0 / §29-6） |
| **运维前置（必须）** | **改真源后必须重启 compat-api** —— **树 JSON 无磁盘指纹**（`lib/store.js:358-374` `getTree()` 仅查进程内 `treeCache`；`:359` 命中即返回、**不 stat**；对比 meta 有指纹 `:272` / `:282` / `:313-315`）；不重启则读侧返旧快照、写侧会把旧快照整份回写。实测回读证据 = `docs/founder-attach.spec.md` **§9-11 (b)** |

**（b）E1 登记行（裁定：不算缺陷 · Zang · 2026-09-20）**：「口径 A（点镜像节点 = 打开真身档案）」在新口径下仍会把镜像坐标 **`replaceState` 收敛到真身**（实测 `?tree_id=ji_23395_01&handle=10400594…` → `?tree_id=ji_23395&handle=3c95530f…`，页面显示 `mirrorNote`「本节点为 季氏祖谱 000163 的镜像 · 内容取自真身」）⇒ **镜像自身的只读档案在正常点击流不可达**；镜像只读判据（`founder-lock` =「始祖节点 · 祖谱镜像」/ `founder-hint` =「始祖节点信息需在本姓祖谱中修改」）**只在内层弹窗 / 不可收敛分支可见**。**Zang 裁定：不算缺陷** —— ① 与 R1 单一真源同向；② **R3 仍是后端防线 + 不可收敛态兜底**；③ **R7 后家族树始祖变真身、不再触发收敛**。⇒ 本册 §16-1 / §16-2（R4 / R5）**均不受影响**（收敛属读侧呈现，不改登记镜像判定与立支 / 汇宗口径）。
...
> **本节（§16-7）边界**：**只改 `docs/**`**；**未改代码、未跑迁移脚本（含 dry-run）、未写真源、未重传 / 部署 / 打包、未提交、未尝试写 `AGENTS.md`**。真源现值 = `config/tree-meta.json` **`13616a89db2782256c3f33260aa32470`**（9,814 B）、`migrate-output/**` **317 文件**。
