...
### 14-13 观察项（**未修 · 只登记 · 非本批缺陷**）：`insufficientBody()` 存在**不可达的第七个 `unit` 键 = `'scroll'`（单数）**

> **性质（只追加 · 2026-09-26）**：本子节**落于 §14 末（§14-12 之后）**、**只追加** —— **§1–§13 与 §14-1 – §14-12 既有行原文一律保留、一字不改、一行不删**。**只登记、不修**（**未改任何代码 / 配置 / `migrate-output/` / `config/`**；**`.qa.md` 一字未动**）。

- **第七键字面（现证）** = `unit = 'scroll'`（**单数**，**非**线上口径的 `'scrolls'`）。产出点 = **`cloudfunctions/compat-api/lib/economy-fee.js` 的 `insufficientBody()`（现证 `NR==631`；归一表达式现证 `NR==637`）** —— 该式对 `bamboo` / `seed` **做了单复数归一**（`bamboo → bamboos`、`seed → seeds`），**对 `scroll` 未归一** ⇒ `err.unit === 'scroll'` 时**原样透传**为响应体 `unit`（经 `errorPayload()` 现证 `NR==665` 到达）。**⚠️ 路径更正（如实登记）**：派单给定路径为 `cloudfunctions/compat-api/lib/economy-ops.js`，**现证该文件无 `insufficientBody` 符号**（`grep -rn "insufficientBody" cloudfunctions/compat-api/lib` ⇒ 命中仅在 `economy-fee.js` 现证 `NR==631` / `NR==665`）⇒ **本子节一律以现证路径为准**。
- **未覆盖事实（现证）**：前端量词单点 = **`frontend/src/business/asset-text.ts` 的 `SHORTAGE_UNIT_BY_KEY`（现证 `NR==235`，表体现证 `NR==236–242`）**，**恰 6 个键** = `fragments` / `seeds` / `bamboos` / `jade` / `scrolls` / `scroll_fragments` ⇒ **无 `'scroll'` 键**；消费点 `shortageLine()`（现证 `NR==251–256`）口径 = **未知 / 缺失键 ⇒ 不加量词、只出数字**（**绝不回退「颗」**）⇒ 若第七键真到前端，明细行**只出数字、不带量词**。
- **可达性核查（三处 `chargeLots(..., 'scroll')` 调用点 · 均不可达响应体）**：① **兰帖分解路由** `POST /assets/scroll/decompose`（`cloudfunctions/compat-api/index.js` 现证 `NR==965–1026`）有**前置容量校验**（现证 `NR==1000–1009`：`count > capacity` ⇒ 抛 `SCROLL_LOCKED_INSUFFICIENT`，`capacity = floor((总片数 − 锁定片数) / 100)`）⇒ 其内部 `decomposeScroll()`（`cloudfunctions/compat-api/lib/economy-ledger.js` 现证 `NR==444`；`chargeLots` 现证 `NR==447`）**不会因不足抛错**；且该路由 catch 分支（现证 `NR==1019–1024`）**自成 body**（仅 `code` / `status` / `message` + 可选 `detail`）⇒ **不经 `errorPayload()` / `insufficientBody()`、不带 `unit`**；② **`grantAssets()`**（`cloudfunctions/compat-api/lib/economy-ops.js` 现证 `NR==534`；`chargeLots(..., 'scroll')` 现证 `NR==581`）有**前置预检**（现证 `NR==577–580`）抛 `scrollsInsufficient()`（现证 `NR==504`），该函数**现证 `NR==510` 已把 `e.unit` 覆写为 `'scrolls'`** ⇒ **单数键经此路径不可达**；③ **好友续约双边扣减**（`cloudfunctions/compat-api/lib/friend-ops.js` 现证 `NR==780`）抛错交 **`runOp()`**（现证 `NR==212`）→ 失败壳（现证 `NR==217–221`）= `{ok:false, error:{code, status, message, reason?, detail?}}` ⇒ **不带 `unit`**。⇒ **三处均不可达响应体 ⇒ 非阻塞、无现网影响**（**无线上 / 现网影响**）。
- **状态 = 未修**（**待裁定**：① 补 `scroll: '片'` 到 `SHORTAGE_UNIT_BY_KEY`，**或** ② 在校验层归一化 `scroll → scrolls`）；**本子节不改任何代码 / 不改量词表 / 不动真源**；**未裁前不得据「存在第七键」判任何实现负**（承 `AGENTS.md` **§0-3**「不写实现现状读数」与 **§0-4** 历史行口径）。
- **判据面（承 `AGENTS.md` §0-3）**：本子节**只登记事实与现证行号**；测试条数 / 通过数一律以对应命令的实际输出为准；**本册删除行 = 0**、**未回改任何历史行**。**同步登记 = `AGENTS.md` §7 追加行**（本册只此一处登记，**不重复写两遍**）。
- **观察项关账（**追加 · 只追加，上条观察项文字一字不改、一律保留**）**：**已修** —— commit **`5f927f1`**（标题「**fix(copy): 资产不足量词单点补单数键 `scroll` ⇒「片」（防御性焊死 · 零行为变更）**」，2026-09-26）。**落点（`awk NR==n` 现证）** = `frontend/src/business/asset-text.ts` —— **`NR==283`**（`export const SHORTAGE_UNIT_BY_KEY: Record<string, string> = {`）· **表体 `NR==284–290`**（**七键** = `fragments` / `seeds` / `bamboos` / `jade` / **`scroll`（`NR==288`）** / `scrolls`（`NR==289`） / `scroll_fragments`（`NR==290`））· **消费点 `shortageLine()` 现证 `NR==300`**（查表单点 `NR==301`）。**依据（逐字须含）**：① **本子节上条「机器可读字段的硬口径」**（`e3a1fa0` 后 `need` / `current` **恒为片** ⇒ **量词跟着数字口径走**）⇒ **第七键量词 = 「片」**（`SCROLL_PIECES_UNIT`，**不得**用「张」）；② **Kevin 2026-09-26 拍定「焊死」**（**该链路现不可达响应体** ⇒ **该键只为将来可达时 UI 不丢量词**，**零行为变更**：只多一个 map 键，既有键 / 量词字面 / import 一字未动）。**⇒ 第七键已补，本子节状态自本行起由「未修」收口为「已修」**；**上条观察项正文（含「状态 = 未修」与「待裁定 ① 补 `scroll: '片'` / ② 在校验层归一化」两案）原文一字不改、一律保留**；**「未裁前不得据『存在第七键』判任何实现负」的禁令随本关账一并失效**（**已修、已焊死**）。**说明（防误引行号）**：上条观察项所引「`SHORTAGE_UNIT_BY_KEY` **现证 `NR==235`**，表体现证 **`NR==236–242`**」与「`shortageLine()` 现证 **`NR==251–256`**」系其**落笔时点**读数，**现证已漂移**（**表体 `NR==284–290`**、**`shortageLine()` `NR==300`**）—— **本次落点行号一律以现证为准**；**上条文字原文保留、不回改**。**本行不改任何代码 / 量词表 / 真源**（**实现 = commit `5f927f1`，归另单**）。

### 14-14 追记：**兰帖残页【取消自动合成】· 改用户【手动合成】**（**追加子节 · Kevin 2026-09-26 当面裁定 · 只追加 · 不改 §14-1 – §14-13 任何行 · §1–§13 删除行 = 0 · 零代码 / 零真源**）

> **性质（只追加）**：本子节 = **§14 之追记**（**落于 §14-13 之后**）。**纯追加** —— **§1–§13 与 §14-1 – §14-13 既有行原文一律保留、一字不改、一行不删**（**含 §14-13 的观察项正文与本次新增的关账行**）；凡被本子节取代者，**只在本子节新增行里标「已被取代 · 原文保留」并写明取代者**（`AGENTS.md` §0-4）。**落盘方 = Jing（制度员）**（`AGENTS.md` §1.1：**无代码修改权限**，只登记口径；**实现 = 另单（Kong）**）。
> **主口径真源 = `docs/friend-domain.spec.md` §25**（**本子节只落运营侧 / 本册侧旧行追记，不复述合成 / 路由 / 流水规格**）；**数值 / 展示面真源 = `docs/economy.spec.md` §21-8**；**同批落点** = `AGENTS.md` **勘误 11** + **§7 追加行** · `docs/PENDING_DEPLOY.md` **§40**。

- **裁定要点（逐字 · 运营侧可见面 · 5 条）**：① **残页正向走 `addScrollFragments` = 纯累加**（**照收、不拒绝、不截断、不自动合成**；返回键 `synthesized` **恒 0**、`scroll_lots` 恒空，**仅兼容保留**）；② **「满 100 自动合成 1 张」机理整体取消**（**`sweep` 不再顺带合成**、**运营发放不再顺带合成**）；③ **合成 = 用户手动**（入口 = 行囊残页格提示层【合成】按钮；**一次 1 张 = 恰耗 100 片、余数保留**；路由 = `POST /assets/scroll/synthesize`）；④ **`SCROLL_FRAGMENT_CAP` = 99 → 999**（**单格容纳上限 / 展示层口径，非拒绝阈值**）；⑤ **`SCROLL_FRAGMENT_SYNTH_THRESHOLD` 保持 100**（**语义 = 手动合成门槛**）。
- **受影响旧行（**现证行号 · 一律原文保留、不回改、不上移**）**：

| 行号（现证） | 该行字面（节选逐字） | 处置 |
|---|---|---|
| **544** | §14-1 表第 6 行 兰帖残页：「**100 片 = 1 张**（**满 100 自动合成**）」 | **已被取代（限括号内「满 100 自动合成」）** ⇒ **100 片 = 1 张（手动合成，一次 1 张）**；**量词「片」与换算 100 不改** |
| **577** | §14-4 快照字段表 `scroll_fragment_cap` 行「**数值以 `docs/economy.spec.md` §16-1 为准，本节不复述**」 | **继续有效（只指针）** ⇒ 该数值**自本批起 = 999**（**经该册 §21-8 更新**；**本行不预填数值**） |
| **642** | §14-9 用例面「六类各一条正向发放 + 一条负向扣减…」 | **继续有效**；**本批追加一条判据语义 = 发放残页不再合成**（**用例面追加，旧行原文保留**） |
| **661** | §14-11「**数值零改动**：本裁定**不改任何数值**（99 / 100 / 100 / 99 一字不改 —— 承 `AGENTS.md` **勘误 2**）」 | **部分取代（限 `SCROLL_FRAGMENT_CAP` 一项）** ⇒ **该数值自本批起 = 999**（**其余 100 / 100 / 99 一字不改**；**旧行原文保留**） |

- **运营侧行为变更（逐字 · 3 条）**：① **`grantAssets` 正向发放残页** ⇒ **只累加、不合成**（`delta.scroll_fragments > 0` 后残页**原样留在标量字段**，**等用户手动合成**；**同批 `scrolls` 不变、无 `scroll_synth` 流水**）；② **「脏数据 ≥ 100 顺带合成」的收口已删**（**发放路径不再有任何合成副作用**）；③ **负向扣减 / 快照五项字段 / 日志六类显示 / 409 `unit` 六类口径** **一律不变**（**本批一字未动**）。
- **存量不迁移（硬 · 逐字）**：**历史 `source: 'scroll_synth'` 批次与历史流水一律保留、不迁移、不改写、不删除**；**仍可被 `chargeLots` / `decomposeScroll` 正常扣减**；**脏数据不清理**（**上线不触发任何批量写盘**）。
- **判据面（承 `AGENTS.md` §0-3）**：本子节**只登记口径与现证行号**；**测试条数 / 通过数 / 构建字节 / 路由条数一律以对应命令的实际输出为准**；**未跑测试 / 未跑构建 / 未打包 / 未部署 / 未重传**。
- **本子节未做项（如实登记）**：**未改任何代码 / 配置 / `migrate-output/` / `config/`**（**真源零写入**、全程只读）；**未碰任何 `.qa.md`**（`AGENTS.md` §0-5）；**未裁任何【待裁】/【未决】项**（含 §14-11 的注销六类，**不受本批影响**）。
- **一句话可改**：改本子节第一条任一处（须同步 `docs/friend-domain.spec.md` **§25** 与 `docs/economy.spec.md` **§21-8**）。
