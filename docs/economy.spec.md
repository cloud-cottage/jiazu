...
## 21. 全站**量词统一**（**追加章节 · Kevin 已裁 2026-09-26 · 只追加 · 不改 §1–§20 任何历史行 · 删除行 = 0**）

> **性质（只追加）**：本节为**纯追加章节** —— **§1–§20 既有行原文一律保留、一字不改、一行不删**；凡被本节取代者**只在本节新增行里标「已被取代 · 原文保留」并写明取代者**（`AGENTS.md` §0-4；`docs/*.qa.md` 不得回改 = §0-5，本单**未碰任何 `.qa.md`**）。**落盘方 = Jing（制度员）**（`AGENTS.md` §1.1：**无代码修改权限**，只登记口径；代码实现归**另单**）。
> **来源**：Kevin **2026-09-26 当面拍定**（本批量词统一表 = §21-1；**同批同表**另见 `docs/economy-ops.spec.md` **§14-1**（资产运维六类）/ `docs/friend-domain.spec.md` **§24**（兰帖与残页））。
> **数值 / 字面纪律**：本节**不写任何实现现状读数**（行数 / 测试条数 / 构建字节 / 常量值一律不写）；凡引用文件、常量、文案**一律以对应命令（`grep` / 源码现证）的实际输出为准**；**行号一律以现证命令为准**。**本节不改任何数值**（量级、上限、换算比一字不动 —— 承 §16-1 的权威值 **99 / 100 / 100 / 99**）。

### 21-1 量词统一表（**逐字 · 本批真源表**）

| 品类 | 真源键 | **新量词（现行）** | 旧量词 | 换算 |
|---|---|---|---|---|
| 石榴籽 | `seeds` | **颗** | 颗（不变） | **1 格 = 9999 颗** |
| 竹片（行囊名「竹简」） | `bamboos` | **片** | 片（不变） | **1 格 = 100 片** |
| 石榴籽碎片 | `fragments` | **片** | ~~个~~ | **10 片 = 1 颗籽**（满 10 自动合成） |
| 石榴籽玉 | `jades` | **枚** | 枚（不变） | **1 枚 = 1 格** |
| **兰帖** | **`scrolls`** | **张** | ~~枚~~ | **1 张 = 100 片**；`scrolls[].qty` **仍以片计**（**存储量级一字不改**） |
| **兰帖残页** | **`scroll_fragments`** | **片** | ~~个~~ | **100 片 = 1 张**（满 100 自动合成） |

- **原则（逐字）**：**碎片类（石榴籽碎片 / 兰帖残页）一律「片」；兰帖一律「张」**；**玉仍「枚」、籽仍「颗」、竹片仍「片」**。
- **「片」在本册是两种角色的同名量词** —— ① **竹片 / 兰帖 / 兰帖残页 / 石榴籽碎片**的**计数单位**；② 兰帖批次的**存储单位**（`qty` / `delta.scrolls` **以片计**）。**「张」只用于兰帖的展示与后台输入**（×100 / ÷100 换算见 `docs/economy-ops.spec.md` §14-3）。
- **量词不改数值**：本批**不动**任何换算常量（`SEEDS_PER_ITEM` / `JADES_PER_ITEM` / `BAMBOO_PIECES_PER_ITEM` / `FRAGMENTS_PER_ITEM` / `SCROLL_PIECES_PER_ITEM` 等，**取值以 §14-1 / §16-1 / §16-2 与源码现证为准**）。
- **「个」的残留语义（不属于本批量词面 · 逐条）**：「**1 个道具**」「**每格 1 个道具**」「**N 格**」是**行囊格数口径**（量词为「格」/「道具」），**本批一字不动**；本批只取代「用『个』数碎片 / 数兰帖残页」的用法。

### 21-2 受影响既有行（**一律标「已被取代 · 原文保留」** · 旧行**不回改**）

**A. 石榴籽碎片 / 竹片 / 籽 的「个」→「片」/「颗」**

| 行号（现证） | 所在节 | 该行字符（节选逐字） | 处置 |
|---|---|---|---|
| **625** | §13-1 换算常量表 | `FRAGMENTS_PER_ITEM` \| **10** \| 石榴籽碎片：**10 个** = 1 个道具 | **已被取代** ⇒「10 个」读作 **10 片** |
| **645** | §13 碎片提示行 | `碎片 \| ${fragments} **个**石榴籽碎片（满 ${FRAGMENTS_PER_ITEM} 自动合成 1 颗石榴籽）` | **已被取代** ⇒ 量词 = **片** |
| **668 / 669 / 670** | §13-2 溢出类型表 | 玉 `枚` / 籽 **`个`** / 竹简 **`个`**（`KIND_ITEM_UNIT`） | **已被取代** ⇒ 籽 **颗**、竹简 **片**（玉不变） |
| **672** | §13-2 渲染句式例 | `溢出 **2 个**标准石榴籽：空间不足，无法持有`（计数单位 `枚` / `个` / `个`） | **已被取代** ⇒ 例字段的计数单位按 §21-1 读取 |
| **751** | §14-1 换算常量表（新表） | `FRAGMENTS_PER_ITEM` \| **10** \| 石榴籽碎片：**10 个** = 1 个道具（… 实测 **1–9 个**，**仍占 1 格**） | **已被取代** ⇒「10 个 / 1–9 个」读作 **片** |
| **771** | §14-2 溢出提示行 | 计数单位 = 玉 `枚`、竹简 / 籽 / 碎片 **`个`**（`KIND_ITEM_UNIT`） | **已被取代** ⇒ 籽 **颗**、竹简 / 碎片 **片** |
| **772** | §14-2 渲染句式例 | `溢出 **2 个**竹简：空间不足，无法持有` | **已被取代** ⇒ 「2 片竹简」 |
| **1251** | §15-4 行囊展示增量 | 「兰帖碎片同口径：`scroll_fragments > 0` 即占 1 格、角标 = 实际个数（**1–9 个**仍占 1 格）」 | **已被取代** ⇒ 片（**「兰帖碎片」名以 §19-3 的现行名「兰帖残页」为准**） |

**B. 兰帖的「枚」→「张」**（**行号以 `grep -n "枚" docs/economy.spec.md` 的现证输出为准 · 该命令现证 = 86 命中**）

| 行号（现证 · 量词位节选） | 所在节 | 处置 |
|---|---|---|
| **289 / 292 / 307** | §4-6 枚举追加登记 | **已被取代** ⇒ 「1 枚成品兰帖」读作 **1 张**（`scroll_synth` 注释 / `scroll_consume` 行 / `ScrollLot.source` 注释；**`delta = { scrolls: -100 }` 等数值字面一字不改**） |
| **1137 / 1145 / 1146 / 1149 / 1150** | **§14-13** 兰帖「格数 / 枚数」 | **已被取代** ⇒ 「**枚数**」读作「**张数（整道具数）**」；**「格数」与「张数」不得混用**、「措辞不得写成格数」的既有纪律**继续有效** |
| **1180 / 1198 / 1206 / 1207 / 1227** | §15-2 / §15-3 | **已被取代** ⇒ 「1 枚成品兰帖」「返还 9 兰帖碎片」类字面**量词改张 / 片**（**数值 100 / 99 由 §16-1 定稿，本批不动**） |
| **1253 / 1276 / 1278 / 1279** | §15-4 | **已被取代** ⇒ 同上（含「满 10 语义 … 合成 1 枚成品兰帖」与两口径澄清的「枚数」） |
| **1307 / 1308 / 1312 / 1314 / 1330 / 1332** | §16-1 / §16-2 | **已被取代（量词面）** ⇒ 「每枚成品兰帖片数」「1 枚 = 100 片 = 1 行囊格」「分解 1 枚 → 99 碎片」中的**枚**读作**张**；**数值 99 / 100 / 100 / 99 与常量名一字不改** |
| **1486 / 1489** | §18-4 D2 落点索引 | **已被取代（量词面）** ⇒ 「枚数」读作「张数」（**四条口径本身继续有效**） |
| **1576 / 1578 / 1582 / 1636 / 1637 / 1639** | §19-5 / §20-1 | **已被取代（量词面）** ⇒ 「可分解枚数」读作「**可分解张数**」（**派生公式与 TTL 口径一字不改**） |
| **1523 / 1544 / 1661** | §19-1 默认序 / §19-3 改名 / §20-3 字面替换登记 | **未取代 · 继续有效**（**该三行不含兰帖量词位**；**逐条以现证命令输出为准**） |

- **完整行号清单（可复现 · 逐条）**：`grep -n "枚" docs/economy.spec.md` 现证 = **86 命中**，行号逐字为 —— `4,5,26,30,79,119,133,236,262,283,289,292,305,307,318,438,538,558,562,605,610,615,623,629,668,672,684,741,749,755,771,772,787,797,798,801,809,879,934,947,1021,1061,1127,1137,1145,1146,1149,1150,1151,1159,1180,1198,1200,1206,1207,1213,1215,1227,1237,1253,1276,1278,1279,1307,1308,1309,1312,1314,1318,1319,1330,1383,1387,1486,1489,1521,1576,1578,1582,1619,1636,1637,1639,1648,1654,1696`。
- **⚠️ 不得据行号清单**机械判定「该行整行失效」：**86 命中里混有非量词用法**（**现证举例**：**4 / 133 / 262 / 283 / 305 / 307 / 318 / 1021 / 1061 / 1127 / 1213 / 1215 / 1387 / 1648 / 1654** 均系「**枚举**」一词；**26 / 30 / 79** 为玉的**基准价值「100 元/枚」**；**438** 为 `/assets/summary` 的「**玉的枚数**」；**684 / 668 一类含玉的「枚」** —— **玉的量词本批不变**）⇒ **处置只针对「用『枚』数兰帖」的量词位**，其余**一字不动**；**逐行判定以该行实际内容为准**（`AGENTS.md` §0-1「代码即事实」/ §0-3 字面纪律）。
- **无「个」量词位的节**：§11（口径登记表）· §12（部署要点）· §17 / §18 / §19 / §20 的**数值与工程纪律各行**——**本批未取代其任何字面**（**除**上表 B 中 §18-4 / §19-5 / §20-1 的量词位）。

### 21-3 与「格数 / 枚数」既有口径的衔接（**§14-13 / §18-4**）

- **旧行原文保留**：§14-13 的**四条口径**（① 格数 = 含余数格；② 枚数 = `floor(总片数 / 100)`；③ `scrolls_item_count` 的口径 = ②；④ 展示层用 ①）与**禁止写法三条**（不得把 `scrolls_item_count` 写作「格数」/ 不得把 250 片写作「2 格」/ 不得据枚数判「漏渲染 1 个道具」）**继续有效**。
- **本批唯一变更 = 量词字面**：② 的 **「枚数」自本批起读作「张数（整道具数）」**（**公式 `floor(总片数 / 100)` 不变**）；**「格数」一词不变**。⇒ **「格数看行囊（含余数格）、张数看接口出参」**（**旧行的「枚数」措辞原文保留**）。
- **等同登记** = `docs/economy-ops.spec.md` **§14-4**（`scrolls_item_count` 字段行的量词口径）。

### 21-4 量词文案落点（**登记 · 指路，不复写实现**）

| 落点 | 说明 |
|---|---|
| `frontend/src/business/asset-text.ts` | 物品**显示名 / 量词**单点（**先例 = `SCROLL_FRAGMENT_NAME = '兰帖残页'`，2026-09-25 Kevin 当面给定**；**以现证 `grep` 为准**） |
| `frontend/src/business/inventory.ts` | 行囊换算常量、默认序、`KIND_ITEM_UNIT`（溢出量词） |
| `frontend/src/business/friends.ts` | 好友域量词文案 |
| `frontend/src/business/tasks.ts` | 任务中心量词文案 |
| `frontend/src/components/asset-inventory/asset-inventory.vue` | 行囊组件渲染字面 |

- **「未单点化」= 上列各文件内的量词字面**（**实现归另单**）；本节**只登记落点**，**不写实现现状读数**（**不复述**具体行号 / 常量值）。

### 21-5 未决与待裁（**本批处置 · 不得加码**）

1. **§20-9 第 4 项（现证行号 = 1743）**「『**返还 N 个兰帖残页**』的**单位词**是否改『**张 / 片**』」—— **本项的单位词部分由本批量词表确定**：**兰帖残页 = 片** ⇒ 现行单位词 = **片**（**派生结论，非新增裁定**）；**该旧行原文保留**；**实现侧字面改动（后端分解 `desc` 与前端浮字的同步）归另单**（**本节不写实现现状**）。
2. **账号注销清空：仍四类 · 待裁**（**兰帖 / 兰帖残页是否纳入清空**未裁）—— 登记落点 = `docs/economy-ops.spec.md` **§14-7**；**未裁前不得据任一读法判任何实现负**。
3. **本批不改任何数值**（数值真源仍 = §16-1 的 99 / 100 / 100 / 99；**§15 旧行原文保留**）。

### 21-6 本节未做项（**如实登记**）

- **未改任何代码 / 配置 / `migrate-output/` / `config/`**（**真源零写入**、全程只读）；**未打包 / 未部署 / 未上传**；**未跑测试 / 未跑构建**。
- **本册 §1–§20 删除行 = 0**、**未回改任何历史行**（含 §13 / §14 的「个」量词行与 §15 / §16 的「枚」量词行）、**未碰任何 `.qa.md`**（`AGENTS.md` §0-5）。
- **未裁任何【待裁】项**（含 §21-5 第 2 条的注销清空）；**未写实现现状读数**（除上列 **现证 `grep` 行号 / 命中数**）。
- **一句话可改**：改本节 §21-1 任一行一处（须同步 `docs/economy-ops.spec.md` §14-1 与 `docs/friend-domain.spec.md` §24 同表）。

### 21-7 追记：**行囊计数措辞一律「格」**（三处落点逐字）+ **「籽不足」明细行量词收敛至 `asset-text.ts` 单点**（**追加子节 · 只追加 · 不改 §21-1 – §21-6 任何行 · §1–§20 删除行 = 0 · 零代码 / 零真源**）

> **性质（只追加）**：本子节 = **§21 之追记**（**承 §21-1 量词统一表 / §21-4 量词文案落点**）。**纯追加** —— **§21-1 – §21-6 既有行原文一律保留、一字不改、一行不删**（**§1–§20 同**）；凡被本子节取代者，**只在本子节新增行里标「已被取代 · 原文保留」并写明取代者**（`AGENTS.md` §0-4「历史版本行不机械改写」）。**本子节对真源零写入**（`migrate-output/` 与 `config/` **未改 / 未移 / 未删**，全程只读）；**未改任何代码 / 配置**（**实现 = 另单，已由 commit `0666140` 落盘，本子节只登记口径与字面**）；**未碰任何 `.qa.md`**（`AGENTS.md` §0-5）。
> **落点说明（防行号漂移 · 逐字）**：本子节**落于 §21 末（§21-6 之后）而不插在 §21-1 – §21-6 之间** ⇒ **§1–§21 既有行号零漂移**（**行号一律现证 · `awk NR==n`**）。
> **来源**：Kevin **2026-09-26 当面裁定**；**实现 = commit `0666140`**（`fix(copy): 行囊「道具」计数措辞统一为「格」+ 立支籽不足明细行量词收敛至 asset-text 单点`；**该 commit 亦含 `20cfa48` 之后的前序追记**）。**落盘方 = Jing（制度员）**（`AGENTS.md` §1.1：**无代码修改权限**，只登记口径；代码实现归**另单**）。

**一、行囊计数措辞一律「格」（不再用「道具」作计数单位）· 三处落点逐字（行号一律现证）**

| # | 落点（`awk NR==n` 现证） | 改前（**已被取代 · 原文保留**） | **改后（逐字 · 现行）** |
|---|---|---|---|
| ① | `frontend/src/components/asset-inventory/asset-inventory.vue` **`NR==11`**（副标题） | `6 × 6 栏位 · 每格 1 个道具 · 拖曳可调整显示顺序（仅当前页面有效，刷新后恢复默认）` | **删去「每格 1 个道具 · 」半句** ⇒ `6 × 6 栏位 · 拖曳可调整显示顺序（仅当前页面有效，刷新后恢复默认）` |
| ② | `frontend/src/business/inventory.ts` **`NR==392`**（余数格提示） | `本格为余数 · 不足 1 个完整道具` | **`本格为余数 · 不足 1 格`** |
| ③ | `frontend/src/business/asset-text.ts` **`NR==126`**（`scrollCaliberLine()` 函数体） | `整道具 ${items} 张` | **`整格 ${items} 张`**（**其余句逐字不动**；**数值口径一字未动**） |

- **判据（一句）**：行囊的**计数单位一律「格」**；「**道具**」仅作**物品类别名词**（如「整堆格 / 余数格」里的物品叫法），**不得再作计数单位**（**故「N 个道具」类写法不再合规**）。
- **未纳入本表者原文保留**：`inventory.ts` **`NR==302`** 的 `本格为余数 · 不足 1 ${KIND_QTY_UNIT.scroll}完整${KIND_NAME.scroll}`（**兰帖条目的另一条渲染分支**）**本批未改、原文保留**（**逐条以现证为准**）。

**二、「籽不足」明细行的量词**单点**收敛（`asset-text.ts`）· 六键表（逐字 · 本批真源表）**

- **裁定（Kevin 2026-09-26）**：`frontend/src/components/person-manage-panel/person-manage-panel.vue` **不再硬编码「颗」**、**不再展示英文 `unit` 键**；**文案 = `本次需 X <量词>，当前可用 Y <量词>`**。
- **单点落点（`awk NR==n` 现证）**：`frontend/src/business/asset-text.ts` —— `SEED_QTY_UNIT = '颗'`（**`NR==219`**）· `JADE_QTY_UNIT = '枚'`（**`NR==224`**）· `SHORTAGE_UNIT_BY_KEY`（**`NR==235–242`**）· `shortageLine()`（**`NR==251–256`**，**唯一拼装点**，行字面 = ``本次需 ${...}，当前可用 ${...}``）。
- **调用方落点（`awk NR==n` 现证）**：`frontend/src/components/person-manage-panel/person-manage-panel.vue` —— **`NR==833`**（函数注释）/ **`NR==834`**（`function seedShortageLines(e: unknown)`）/ **`NR==838`**（量词单点注释）/ **`NR==839`**（`lines.push(shortageLine(err?.need, err?.current, err?.unit));`）⇒ **组件内零「颗」/「枚」量词字面、不展示英文 `unit` 键**。
- **后端 `unit` 键 → 中文量词（六键 · 逐字）**：

| 后端 `unit` 键 | 中文量词 | 依据（`asset-text.ts` 常量） |
|---|---|---|
| `fragments`（石榴籽碎片） | **片** | `SEED_FRAGMENT_UNIT`（复用既有字面） |
| `seeds`（石榴籽） | **颗** | `SEED_QTY_UNIT`（本单新字面） |
| `bamboos`（竹片） | **片** | `BAMBOO_PIECES_UNIT`（复用既有字面） |
| `jade`（石榴籽玉 · **单数键，既有字面**） | **枚** | `JADE_QTY_UNIT`（本单新字面） |
| **`scrolls`（兰帖）** | **片** | `SCROLL_PIECES_UNIT`（复用既有字面；**理由见下条三**） |
| `scroll_fragments`（兰帖残页） | **片** | `SEED_FRAGMENT_UNIT`（复用既有字面） |

- **未知 / 缺失键 ⇒ 不加量词**（**只出数字**，行文 = `本次需 X，当前可用 Y`，**不留悬空空格**）；**绝不回退「颗」**（**硬口径**；`SHORTAGE_UNIT_BY_KEY` 之外无默认值）。
- **数字缺失**（`undefined` / `null`）⇒ `—`（与既有 `?? '—'` 口径一致）。
- **`KIND_QTY_UNIT.seed`（`inventory.ts`）不在本单点内** —— 那是**行囊格内单位表**（**另一条链路**），本轮**未纳入**（**原文保留**；**落点以现证为准**）。

**三、必写口径句（**重要**）：`scrolls` 在此处**必须为「片」而非「张」**

- **理由（一句）**：`need` / `current` **自 commit `e3a1fa0` 起恒为「片」**（与 `unit = 'scrolls'`、**线上存储（`scrolls[].qty` / `scroll_lots[].qty`）**、**请求 `delta.scrolls`** **同分母**）⇒ **量词跟着数字口径走** ⇒ **「片」**。
- **「张」只活在人文 `message` 里**（**参 `docs/economy-ops.spec.md` §14-12**；**「张」不得进机器可读字段**）；**不得写「1.5 张」这类假张数**；**不得同一响应体内两套单位**。
- **与 §21-1 兰帖「张」行的分工（原文保留 · 不冲突）**：**§21-1 的兰帖「张」= 展示 / 后台输入的量词**（1 张 = 100 片，×100 / ÷100 换算见 `docs/economy-ops.spec.md` §14-3）；**本子节的「片」= 资产不足（`ASSET_INSUFFICIENT`）明细行 `need` / `current` 的量词** ⇒ **同一 `scrolls` 键、两条链路两条量词，各自以其数字的实际单位为准**（**§21-1 兰帖「张」行原文一字不改**）。

**四、旧行原文保留声明（**不得回改**）**

- **§21-1 末条（「个」的残留语义 · 现证行号 `NR==1777`）**：该行「**「1 个道具」「每格 1 个道具」「N 格」是行囊格数口径（量词为「格」/「道具」），本批一字不动**」**原文一律保留**；其**「`道具` 可作计数单位」的语义自本追记起按新口径读取** —— **计数单位一律「格」**（**「道具」仅作物品类别名词**）。**不得回改 `NR==1777`**、**不得据该行判实现负**。
- **本册 §1–§21-6 删除行 = 0**、**未回改任何历史行**（含 §21-1 兰帖「张」行与「格数 / 张数」各条）；**未碰任何 `.qa.md`**（`AGENTS.md` §0-5）；**未写实现现状读数**（**除上列现证行号 / commit hash 外，本节不写测试条数 / 构建字节 / 常量取值**）。

**五、本节未做项（如实登记）**

- **未改任何代码 / 配置 / `migrate-output/` / `config/`**（**真源零写入**、全程只读）；**未打包 / 未部署 / 未上传**；**未跑测试 / 未跑构建**（**实现与读数归另单，见 `AGENTS.md` §7 本单回填行**）。
- **未裁任何【待裁】项**；**§21-5 三项处置一字不改**。

---

### 21-8 追记：兰帖残页**【取消满 100 自动合成 · 改用户手动合成】** + **数值 `SCROLL_FRAGMENT_CAP` 99 → 999** + **行囊展示 = 每 999 片 1 格**（**追加子节 · Kevin 2026-09-26 当面裁定 · 只追加 · 不改 §21-1 – §21-7 任何行 · §1–§20 删除行 = 0 · 零代码 / 零真源**）

> **性质（只追加）**：本子节 = **§21 之追记**（**落于 §21-7 之后**）。**纯追加** —— **§21-1 – §21-7 既有行原文一律保留、一字不改、一行不删**；凡被本子节取代者，**只在本子节新增行里标「已被取代 · 原文保留」并写明取代者**（`AGENTS.md` §0-4）。**落盘方 = Jing（制度员）**（**无代码修改权限**，只登记口径；**实现 = 另单（Kong）**）。
> **⚠️ 取代关系（本子节头部明写 · 逐字）**：**① 展示面** —— 本子节**取代本册内一切「兰帖残页（兰帖碎片）`> 0` 即占 1 格 / 1–9 个仍占 1 格」的写法**（**含 §15-6 ① 与 §21-2 A 表内引该行的两处**）⇒ **自本子节起，行囊内兰帖残页 = 每 999 片占 1 格**；**② 数值面** —— 本子节**取代 §16-1 的 ① 项（兰帖碎片上限 = 99）为 999**；**③ 合成机理** —— **「满 100 自动合成」整体取消，改用户手动合成**（**规格面真源 = `docs/friend-domain.spec.md` §25**，本子节**不复述合成 / 路由 / 流水规格**）。**上列旧行一律原文保留、不回改、不上移**；其**语义自本子节起按本子节读取**。
> **来源**：Kevin **2026-09-26 当面裁定**（① **取消【兰帖残页】满 100 自动合成**；② **改用户手动合成**）；**主口径真源 = `docs/friend-domain.spec.md` §25**（好友域侧主册）；**同批落点** = `docs/economy-ops.spec.md` **§14-14** + **§14-13 关账行** · `AGENTS.md` **勘误 11** + **§7 追加行** · `docs/PENDING_DEPLOY.md` **§40**。
> **数值 / 字面纪律（承 `AGENTS.md` §0-3）**：本子节**不写实现现状读数**（测试条数 / 通过数 / 构建字节 / 常量“现盘值”一律以现证命令为准）；**行号一律以现证命令（`grep -n` / `awk NR==n`）为准**。

- **裁定（逐字 · 本册面）**：**行囊每 999 片兰帖残页占 1 格**；**占格数 = `ceil(片数 / 999)`**；**0 片不占格**；**超出另起一格**；**整格角标 = 999**、**余数格角标 = 余数**（沿用既有**余数格**体例：`slotKind = 'remainder'`、该类内排在最后、余数格提示行照旧）。
- **单点常量（前端 · 逐字 · 现证 `awk NR==n`）**：`frontend/src/business/inventory.ts` 的 **`SCROLL_FRAGMENT_PIECES_PER_CELL = 999`**（**`999` 的唯一字面**；**现证 `NR==60`**）；装配函数 = **`scrollFragmentItems()`**（**现证 `NR==329`**；**占格数 `NR==332`**、**余数 `NR==333`**）；**合成门槛常量 = `SCROLL_FRAGMENTS_PER_ITEM` = 100（不变）**。
- **语义（硬 · 不得混用）**：`999` **不是拒绝阈值** —— **后端不做上限校验、不拒绝、不截断**；**`SCROLL_FRAGMENT_CAP` = 单格容纳上限 / 展示层口径**。
- **数值链（承 §16-1 · **只改一项**）**：§16-1 的 **① 兰帖碎片上限 = 99** ⇒ **自本子节起 = 999**（**② 合成阈值 100 / ③ 每张片数 100 / ④ 分解返还 99 一字不改**）。**§16-1 节标题内「权威值 99 / 100 / 100 / 99」字样原文保留**（**该节现证行号 = 1299**）；**本子节即该 ① 项的取代落点**。
- **合成门槛语义（逐字）**：`SCROLL_FRAGMENT_SYNTH_THRESHOLD` **保持 100** —— **语义 = 手动合成门槛**（合成 1 张所耗片数），**不再表示「自动触发阈值」**（**该常量取值不改**；**合成 = 用户手动**，见 `docs/friend-domain.spec.md` **§25**）。
- **「格数 / 张数」两口径纪律继续有效**（承 §14-13 与 §18-4）：**「格数」看行囊（含余数格）、「张数（整道具数）」看接口出参 `scrolls_item_count`** —— **本批不动**；**兰帖（`scrolls`）自身的换算与占格（1 张 = 100 片 / 余数占格）一字不改**（**本子节只改残页一类**）。
- **受影响旧行（**现证行号 · 一律原文保留、不回改、不上移**）**：

| 行号（现证） | 该行字面（节选逐字） | 处置 |
|---|---|---|
| **1251** | §15-6 ① 碎片类表「占格」行：「**兰帖碎片同口径**：`scroll_fragments > 0` 即**占 1 格**、角标 = 实际个数（**1–9 个仍占 1 格**）」 | **已被取代** ⇒ **每 999 片占 1 格**（**「兰帖碎片」名以 §19-3 的现行名「兰帖残页」为准**） |
| **1792** | §21-2 A 表内引 **1251** 行的那一行（处置列 = 「**已被取代** ⇒ 片」） | **部分取代** ⇒ **该行对 1251 的「片」量词处置继续有效**；**其「占 1 格」语义自本子节起按「每 999 片 1 格」读取** |
| **1772** | §21-1 量词表 兰帖残页行「**100 片 = 1 张**（满 100 自动合成）」 | **已被取代（限括号内「满 100 自动合成」）** ⇒ **100 片 = 1 张（手动合成，一次 1 张）**；**量词「片」与换算 100 不变** |
| **1306** | §16-1 表 ① 行「兰帖碎片上限 | **99** | **`SCROLL_FRAGMENT_CAP`** | 9」 | **已被取代** ⇒ **999**（**取代者 = 本子节 + `docs/friend-domain.spec.md` §25-5**；**旧值 99 字样原文保留**） |
| **1314** | §16-1 尾「**定稿数值链**：…**满 100 碎片立即自动合成 1 枚成品兰帖（碎片取余）**…」 | **部分取代** ⇒ **「满 100 立即自动合成」一段已被取代**（**改用户手动合成**）；**该链的换算与数值 100 / 99 不变** |
| **289 / 307** | §4-6 枚举追加登记（`Tx.type` 的 `scroll_synth` 注释 / `ScrollLot.source` 的 `scroll_synth` 注释）「兰帖碎片满 100 自动合成 1 枚成品兰帖」 | **部分取代** ⇒ **触发方 = 用户手动**（**枚举字面 `scroll_synth` 不变**） |
| **1761** | §21 引言「**数值 / 字面纪律**…**本节不改任何数值**（**权威数值 = 本册 §17-8 / `docs/economy.spec.md` §16-1**）」 | **部分取代（限 `SCROLL_FRAGMENT_CAP` 一项）** ⇒ **该数值自本批起 = 999**（**其余数值与量词纪律继续有效**；**旧行原文保留**） |
| **1833** | §21-5 第 3 条「**本批不改任何数值**（数值真源仍 = §16-1 的 99 / 100 / 100 / 99；**§15 旧行原文保留**）」 | **部分取代（限 `SCROLL_FRAGMENT_CAP` 一项）** ⇒ **该数值自本批起 = 999**（**其余三值不变**；**旧行原文保留**） |

- **判据面（承 `AGENTS.md` §0-3）**：本子节**只登记口径与现证行号**；**测试条数 / 通过数 / 构建字节 / 产物 md5 一律以对应命令的实际输出为准**；**未跑测试 / 未跑构建 / 未打包 / 未部署 / 未重传**。
- **本子节未做项（如实登记）**：**未改任何代码 / 配置 / `migrate-output/` / `config/`**（**真源零写入**、全程只读）；**§21-1 – §21-7 与 §1–§20 删除行 = 0**、**未回改任何历史行**；**未碰任何 `.qa.md`**（`AGENTS.md` §0-5）；**未裁任何【待裁】/【未决】项**。
- **一句话可改**：改本子节首条「每 999 片占 1 格」一处（须同步 `docs/friend-domain.spec.md` **§25-5** 与 `docs/economy-ops.spec.md` **§14-14** + `AGENTS.md` **勘误 11**）。
