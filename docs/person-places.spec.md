...
## 19. 反向指针：家谱 tab 组件化批次对本册 **`D3`** 的取代（**追加 · 2026-09-24 · 只追加 · 上列各节原文一律保留**）

> **性质**：**追加式反向指针行** —— **未改代码、未写真源、未改 `AGENTS.md`**；本册 §1 – §18 的历史行**一字未动**（承本册「只追加」纪律），下列各项的语义**自本节起被覆盖**（口径 = `AGENTS.md` §0 第 4 条「历史版本行不机械改写」）。

1. **被取代项 = 本册 §8-1 落点表的 `D3`**（「家族页（`family`）头图 hero」，锚点 = `frontend/src/pages/family/index.vue:24`：`:v-if="hallInfo?.origin"` + `发源地：{{ hallInfo.origin }}`）：**2026-09-24 家谱 tab 单真源组件化批次**已移除该页 **hero 卡** ⇒ **`D3` 该档不再适用**；该行**原文保留**。
2. **不受影响 = `D1`（首页卡片）/ `D2`（家族树页头）**：两者**继续读 `tree-meta.origin`**、判定不变；本册 **§8-1** 与 **§6-3** 中凡「首页卡片 / 家族树页头 / 家族页 hero」三档并列处（例：§6-3 的「展示」行），**仅「家族页 hero」一档失效**。
3. **取代关系唯一登记处** = `docs/home-sort-search.spec.md` **§11-4 第 2 行**；`docs/geo-origin.spec.md` **§13-14** 为同项反向指针（**两册各登记一条、同一条口径，非两套**）。
4. **部署面**：家谱 tab 本批 **零后端改动** ⇒ **无云函数重打包项**；**前端 H5 + 小程序必须重打** ⇒ 登记 = `docs/PENDING_DEPLOY.md` **§32-7**。

> **本节边界**：追加式登记 —— **未改代码、未写真源、未执行任何打包 / 部署 / 上传 / 重传、未 commit**；本册上列各节**一律原文保留**。
